...
#define CYBSP_2V8_EN_PIN 		(P5_6)  // 显示屏2.8V
...
void driver_rm69600_write_data(uint8_t data)
{
    ...
    uint32_t ticks;
//    BOOT_LOG_INF("driver_rm69600_write_data wait tx last dong!");
    while(tx_dma_done==false);
    tx_dma_done= false;
    while (true == Cy_SCB_SPI_IsBusBusy(CYBSP_AMOLED_SPI_HW));
    g_send_one_data = data;
...
cy_rslt_t rslt;
cy_stc_scb_spi_context_t SPI_AMOLED_context;
...
//    driver_amoled_power_up(1); // 先初始化上电时序，以免初始化SPI时因为SPI的IO串电到IOVCC
    ...
    status = Cy_GPIO_Pin_Init(CYBSP_SPI_MOSI_PORT, CYBSP_SPI_MOSI_PIN, &CYBSP_SPI_MOSI_config);
    if (status != CY_GPIO_SUCCESS) {
        return -1;
    }
    status = Cy_GPIO_Pin_Init(CYBSP_SPI_MISO_PORT, CYBSP_SPI_MISO_PIN, &CYBSP_SPI_MISO_config);
    if (status != CY_GPIO_SUCCESS) {
        return -1;
    }
    status = Cy_GPIO_Pin_Init(CYBSP_SPI_SCK_PORT, CYBSP_SPI_SCK_PIN, &CYBSP_SPI_SCK_config);
    if (status != CY_GPIO_SUCCESS) {
        return -1;
    }
    status = Cy_GPIO_Pin_Init(CYBSP_SPI_CSN_PORT, CYBSP_SPI_CSN_PIN, &CYBSP_SPI_CSN_config);
    if (status != CY_GPIO_SUCCESS) {
        return -1;
    }
...
//    sm_log(SM_LOG_INFO,"SPI init succ! \r\n");
    ...
    cy_stc_sysint_t cfg =
    {
        ...
        .intrPriority = 2,
        .cm0pSrc = (cy_en_intr_t)CYBSP_AMOLED_SPI_DMA_TX_IRQ,
    };
    Cy_SysInt_Init(&cfg, (cy_israddress)tx_dma_complete);

     NVIC_EnableIRQ((IRQn_Type)cfg.intrSrc);
