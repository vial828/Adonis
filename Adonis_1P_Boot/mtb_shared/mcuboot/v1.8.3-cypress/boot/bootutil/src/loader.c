...
boot_is_header_valid(const struct image_header *hdr, const struct flash_area *fap)
{
    ...
    BOOT_LOG_ERR("hdr->ih_img_size: %lu, hdr->ih_hdr_size:%lu", hdr->ih_img_size, hdr->ih_hdr_size);
    ...
    if (size >= flash_area_get_size(fap)) {
        BOOT_LOG_ERR("Image get_size err");
        return false;
    }
    #endif
    #if 1 // test 20240511
    get_size= flash_area_get_size(fap);
    BOOT_LOG_ERR("Image size: %lu, get_size:%lu", size, get_size);
    if (size >= get_size) {
        BOOT_LOG_ERR("Image get_size err");
        return false;
    }
...
#if !defined(MCUBOOT_DIRECT_XIP) && !defined(MCUBOOT_RAM_LOAD)
extern int driver_amoled_init(void);
uint8_t gLoadingStatus = 0;
/**
...
#endif
    driver_mstick_init();
    BOOT_LOG_INF("OLED Init!");
    driver_amoled_init(); // 初始化显示屏，by huangxuhua 20250210
...
boot_copy_image(struct boot_loader_state *state, struct boot_status *bs)
{
    ...
    sect_count = boot_img_num_sectors(state, BOOT_PRIMARY_SLOT); // by xuhua.huang 应该根据header中的实际文件大小，实际使用多少，拷贝多少sector 而不是根据最大sector来 以缩短搬运时间
...
#if 0 // by xuhua.huang boot_copy_region中的 Cy_Flash_WriteRow有擦除动作，跳过单独擦除动作
        rc = boot_erase_region(fap_primary_slot, size, this_size); // 20 S
        assert(rc == 0);
#endif
    ...
    rc = boot_copy_region(state, fap_secondary_slot, fap_primary_slot, 0, 0, size); //36S
    if (rc != 0) {
        return rc;
    }
	
// 返回错误，先屏蔽, 由于运行区不进行验证，没必要再写magic
#if 0
#if defined(MCUBOOT_OVERWRITE_ONLY_FAST)
    rc = boot_write_magic(fap_primary_slot);
    if (rc != 0) {
        BOOT_LOG_INF("Image upgrade secondary boot_write_magic %d!", rc);
        return rc;
    }
#endif
#endif
...
context_boot_go(struct boot_loader_state *state, struct boot_rsp *rsp)
{
    ...
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        ...
        BOOT_LOG_DBG("***********app_has_up %d**********************", has_upgrade);
    }
...
//wanggankun@2024-8-20, note, Deal with upgrade file
fih_int
context_boot_go(struct boot_loader_state *state, struct boot_rsp *rsp)
{
