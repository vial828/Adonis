 ...
 * @param[in] min_hf_freq           minimal acceptable HF clock frequency to consider.
...
cy_rslt_t _cyhal_utils_find_hf_source_n_divider(cyhal_clock_t *clock, uint32_t hz,
                        ...
                        cyhal_clock_t *hf_source, uint32_t *div, uint32_t min_hf_freq);
...
/** Enable the IP block that drives the specified item.
 * @param[in] rsc  The item to get the source IP block enable for
 */
void _cyhal_utils_enable_ip_block(cyhal_resource_inst_t *rsc);

/** Calculate max clock divider for a given clock block
 * @param[in] block The clock block that max div calculation takes place for.
 */
