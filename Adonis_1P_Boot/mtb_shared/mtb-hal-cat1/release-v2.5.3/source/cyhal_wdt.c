...
cy_rslt_t cyhal_wdt_init(cyhal_wdt_t *obj, uint32_t timeout_ms)
{
    ...
    //Create resource element to be able to call enable ip block afterwards
    //There is no resource for WDT so use RTC as it triggers the enablement we need
    #if defined(COMPONENT_CAT1D)
    cyhal_resource_inst_t rsc = {CYHAL_RSC_RTC, 0, 0};
