...
#define _CYHAL_DMAC_MAX_EL_PER_TRANSFER             (65536UL)
#elif defined(CY_IP_MXSAXIDMAC)
#define _CYHAL_DMAC_MAX_EL_PER_TRANSFER             (CY_AXIDMAC_LOOP_COUNT_MAX)
#else
#define _CYHAL_DMAC_MAX_EL_PER_TRANSFER             (CY_DMAC_LOOP_COUNT_MAX)
#endif
...
cy_rslt_t _cyhal_dma_dmac_configure(cyhal_dma_t *obj, const cyhal_dma_cfg_t *cfg)
{
    ...
    // DMAC only supports <= _CYHAL_DMAC_MAX_EL_PER_TRANSFER (65536) byte burst and <=_CYHAL_DMAC_MAX_EL_PER_TRANSFER bytes per burst
    if ((cfg->burst_size > _CYHAL_DMAC_MAX_EL_PER_TRANSFER) ||
        ((cfg->burst_size <= 1) && (cfg->length > _CYHAL_DMAC_MAX_EL_PER_TRANSFER)) ||
        ((cfg->burst_size > 0) && (cfg->length > (cfg->burst_size * _CYHAL_DMAC_MAX_EL_PER_TRANSFER))))
...
#if defined(CY_IP_M4CPUSS_DMAC) || defined(CY_IP_M7CPUSS_DMAC) || defined(CY_IP_MXAHBDMAC) || defined(CY_IP_MXSAXIDMAC)
#if defined(CY_IP_MXSAXIDMAC)
static cy_en_axidmac_trigger_type_t _cyhal_dma_dmac_convert_input_t(cyhal_dma_input_t input)
{
    switch(input)
    {
...
#elif defined(CY_IP_M4CPUSS_DMAC) || defined(CY_IP_M7CPUSS_DMAC) || defined(CY_IP_MXAHBDMAC)
static cy_en_dmac_trigger_type_t _cyhal_dma_dmac_convert_input_t(cyhal_dma_input_t input)
{
    switch(input)
    {
        case CYHAL_DMA_INPUT_TRIGGER_SINGLE_ELEMENT:
...
#if defined(CY_IP_MXSAXIDMAC)
static cy_en_axidmac_trigger_type_t _cyhal_dma_dmac_convert_output_t(cyhal_dma_output_t output)
{
    switch(output)
    {
...
#else
static cy_en_dmac_trigger_type_t _cyhal_dma_dmac_convert_output_t(cyhal_dma_output_t output)
{
    switch(output)
    {
        case CYHAL_DMA_OUTPUT_TRIGGER_SINGLE_ELEMENT:
...
cy_rslt_t _cyhal_dma_dmac_connect_digital(cyhal_dma_t *obj, cyhal_source_t source, cyhal_dma_input_t input)
{
    ...
    obj->descriptor_config.dmac.triggerInType = _cyhal_dma_dmac_convert_input_t(input);
...
cy_rslt_t _cyhal_dma_dmac_enable_output(cyhal_dma_t *obj, cyhal_dma_output_t output, cyhal_source_t *source)
{
    ...
    obj->descriptor.dmac.ctl |= _VAL2FLD(_CYHAL_DMAC_CH_DESCR_CTL_TR_OUT_TYPE, _cyhal_dma_dmac_convert_output_t(output));
...
uint32_t _cyhal_dma_dmac_get_max_elements_per_burst(cyhal_dma_t *obj)
{
    CY_UNUSED_PARAMETER(obj);
    return _CYHAL_DMAC_MAX_EL_PER_TRANSFER;
}
