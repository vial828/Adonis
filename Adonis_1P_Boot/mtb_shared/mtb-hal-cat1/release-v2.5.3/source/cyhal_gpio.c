...
/* Define IOSS interrupt number for Secure or Non-secure image */
#if defined(COMPONENT_CAT1D)
#if defined(_CYHAL_IMAGE_TYPE_SECURE)
#define _CYHAL_GPIO_INTERRUPTS_PORT_0_IRQN      (ioss_interrupts_sec_gpio_0_IRQn)
#define _CYHAL_GPIO_INTERRUPTS_PORT_2_IRQN      (ioss_interrupts_sec_gpio_2_IRQn)
#define _CYHAL_GPIO_INTERRUPTS_PORT_5_IRQN      (ioss_interrupts_sec_gpio_5_IRQn)
#define _CYHAL_GPIO_INTERRUPTS_PORT_21_IRQN     (ioss_interrupts_sec_gpio_21_IRQn)
#else
#define _CYHAL_GPIO_INTERRUPTS_PORT_0_IRQN      (ioss_interrupts_gpio_0_IRQn)
#define _CYHAL_GPIO_INTERRUPTS_PORT_2_IRQN      (ioss_interrupts_gpio_2_IRQn)
#define _CYHAL_GPIO_INTERRUPTS_PORT_5_IRQN      (ioss_interrupts_gpio_5_IRQn)
#define _CYHAL_GPIO_INTERRUPTS_PORT_21_IRQN     (ioss_interrupts_gpio_21_IRQn)
#endif /* defined(_CYHAL_IMAGE_TYPE_SECURE) */
...
#if defined(COMPONENT_CAT1D)
    uint32_t port;
    if(irqn >= _CYHAL_GPIO_INTERRUPTS_PORT_0_IRQN && irqn < _CYHAL_GPIO_INTERRUPTS_PORT_2_IRQN)
    {
        port = (uint32_t)(irqn - _CYHAL_GPIO_INTERRUPTS_PORT_0_IRQN);
    }
    else if(irqn >= _CYHAL_GPIO_INTERRUPTS_PORT_2_IRQN && irqn < _CYHAL_GPIO_INTERRUPTS_PORT_5_IRQN)
    {
        port = (uint32_t)(irqn - _CYHAL_GPIO_INTERRUPTS_PORT_0_IRQN + 1);
    }
    else if(irqn >= _CYHAL_GPIO_INTERRUPTS_PORT_5_IRQN && irqn <= _CYHAL_GPIO_INTERRUPTS_PORT_21_IRQN)
    {
        port = (uint32_t)(irqn - _CYHAL_GPIO_INTERRUPTS_PORT_0_IRQN + 2);
    }
    else
    {
        port = 0u;
...
cy_rslt_t cyhal_gpio_init(cyhal_gpio_t pin, cyhal_gpio_direction_t direction, cyhal_gpio_drive_mode_t drive_mode, bool init_val)
{
    ...
    _cyhal_utils_enable_ip_block(&pinRsc);
#else
    ...
    #if defined(_CYHAL_IMAGE_TYPE_SECURE)
        Cy_GPIO_Pin_SecFastInit(CYHAL_GET_PORTADDR(pin), CYHAL_GET_PIN(pin), pdl_drive_mode, init_val, HSIOM_SEL_GPIO);
    #else
...
void cyhal_gpio_free(cyhal_gpio_t pin)
{
    if(_cyhal_gpio_arrays_initialized)
    {
        ...
        #if defined(_CYHAL_IMAGE_TYPE_SECURE)
            Cy_GPIO_Pin_SecFastInit(CYHAL_GET_PORTADDR(pin), CYHAL_GET_PIN(pin), CY_GPIO_DM_ANALOG, 0UL, HSIOM_SEL_GPIO);
        #else
...
#if defined(COMPONENT_CAT1D)
    /* Port 1 and 4 are controlled by SMIF IP and they are in a separate group
     *  which means the IOSS port numbers are non-contiguous - and die specific.
     *  Also GPIO HAL does not operate with pins on ports 1 and 4 because they
     *  require to operate with SMIF IP which is not a part of IOSS IP */
    uint32_t port = CYHAL_GET_PORT(pin);
    _cyhal_system_irq_t irqn;
    if(port < 2u)
    {
        irqn = (_cyhal_system_irq_t)(_CYHAL_GPIO_INTERRUPTS_PORT_0_IRQN + port);
    }
    else if(port < 5u)
    {
        irqn = (_cyhal_system_irq_t)(_CYHAL_GPIO_INTERRUPTS_PORT_0_IRQN + (port - 1u));
    }
    else if(port <= 21u)
    {
        irqn = (_cyhal_system_irq_t)(_CYHAL_GPIO_INTERRUPTS_PORT_0_IRQN + (port - 2u));
    }
    else
    {
