...
#define _CYHAL_LPCOMP_INSTANCES         CY_IP_MXS22LPCOMP_INSTANCES

#define _cyhal_lpcomp_init(base, channel, config) \
    Cy_LPComp_Init_Ext((base), (channel), (config), &(obj->context))

#define _cyhal_lpcomp_enable(base, channel) \
    Cy_LPComp_Enable_Ext((base), (channel), &(obj->context))

#define _cyhal_lpcomp_disable(base, channel) \
    Cy_LPComp_Disable_Ext((base), (channel), &(obj->context))

#define _cyhal_lpcomp_setinterrupttriggermode(base, channel, intType) \
    Cy_LPComp_SetInterruptTriggerMode_Ext((base), (channel), (intType), &(obj->context))
...
static cy_rslt_t _cyhal_comp_lp_convert_power(cyhal_power_level_t hal_power, cy_en_lpcomp_pwr_t* pdl_power)
{
    ...
    switch(hal_power)
    {
        ...
        default:
            ...
            break;
#elif defined(CY_IP_MXLPCOMP_INSTANCES) || defined (CY_IP_MXS22LPCOMP_INSTANCES)
...
cy_rslt_t _cyhal_comp_lp_init(cyhal_comp_t *obj, cyhal_gpio_t vin_p, cyhal_gpio_t vin_m, cyhal_gpio_t output, cyhal_comp_config_t *cfg)
{
	...
	const cyhal_resource_pin_mapping_t *output_map = NULL; // Output pin is not supported so let it fail below if defined
#else
    ...
    if (NULL != vin_p_map)
    {
        ...
        if (result == CY_RSLT_SUCCESS)
        {
            ...
            if (result == CY_RSLT_SUCCESS)
            {
                ...
                Cy_LPComp_SetPower_Ext(obj->base_lpcomp, _CYHAL_COMP_GET_CHANNEL(obj->resource.channel_num), comp_lp_config.power, &(obj->context));
#else
                /* Set channel 0 or 1 power mode - Ultra Low Power mode */
...
cy_rslt_t _cyhal_comp_lp_set_power(cyhal_comp_t *obj, cyhal_power_level_t power)
{
    ...
    {
#if defined (CY_IP_MXS22LPCOMP_INSTANCES)
        Cy_LPComp_SetPower_Ext(obj->base_lpcomp, _CYHAL_COMP_GET_CHANNEL(obj->resource.channel_num), power_level, &(obj->context));
#else
