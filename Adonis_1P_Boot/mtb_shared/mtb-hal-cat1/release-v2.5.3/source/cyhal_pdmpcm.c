...
#define _CYHAL_PDM_MAX_HF_CLK_HZ               (200000000u)
/* The maximum tolerance of desierd HF clock in Hz */
#define _CYHAL_PDM_HF_CLK_TOLERANCE_HZ         (25000000u)
...
#if (_CYHAL_PDM_INST0)
#define _CYHAL_PDM_CHANNELS0 (PDM_NR)
#endif
#if (_CYHAL_PDM_INST1)
...
static _cyhal_system_irq_t _cyhal_pdm_pcm_irq_n0[] =
{
    ...
    #if ((_CYHAL_PDM_CHANNELS0) > 2)
        pdm_0_interrupts_2_IRQn,
    #endif
    #if ((_CYHAL_PDM_CHANNELS0) > 3)
        pdm_0_interrupts_3_IRQn,
    #endif
    #if ((_CYHAL_PDM_CHANNELS0) > 4)
        pdm_0_interrupts_4_IRQn,
    #endif
    #if ((PDM_CYHAL_PDM_CHANNELS0_NR) > 5)
        pdm_0_interrupts_5_IRQn,
    #endif
...
static void _cyhal_pdm_pcm_get_block_from_irqn(_cyhal_system_irq_t irqn, uint8_t* block, uint8_t* channel)
{
    ...
    case pdm_0_interrupts_1_IRQn:
        ...
        *block = 0;
        break;
    #if ((_CYHAL_PDM_CHANNELS0) > 2)
        case pdm_0_interrupts_2_IRQn:
            *channel = 2;
            *block = 0;
            break;
    #endif
    #if ((_CYHAL_PDM_CHANNELS0) > 3)
        case pdm_0_interrupts_3_IRQn:
            *channel = 3;
            *block = 0;
            break;
    #endif
    #if ((_CYHAL_PDM_CHANNELS0) > 4)
        case pdm_0_interrupts_4_IRQn:
            *channel = 4;
            *block = 0;
            break;
    #endif
    #if ((_CYHAL_PDM_CHANNELS0) > 5)
        case pdm_0_interrupts_5_IRQn:
            *channel = 5;
            *block = 0;
            break;
    #endif
...
static cyhal_pdm_pcm_t* _cyhal_pdm_pcm_find_existing_obj(uint8_t block_num)
{
    ...
    while (NULL == existing_obj && i < _cyhal_pdm_num_channels[block_num])
    {
...
static inline void _cyhal_pdm_pcm_set_rx_fifo_level(cyhal_pdm_pcm_t *obj, uint8_t fifo_level)
{
    ...
    if(paired_channel >= 0)
    {
        PDM_PCM_RX_FIFO_CTL(obj->base,paired_channel) = _VAL2FLD(PDM_CH_RX_FIFO_CTL_TRIGGER_LEVEL, fifo_level - 1);
    }
#endif
    ...
    if (obj != NULL)
    {
        ...
        if((CYHAL_PDM_PCM_RX_HALF_FULL & event) || (CYHAL_PDM_PCM_RX_OVERFLOW & event))
        {
            ...
            else
            {
                ...
                for (uint16_t i = 0; i < _CYHAL_PDM_PCM_STABILIZATION_FS; i++)
                {
#if defined(CY_IP_MXAUDIOSS)
...
static inline cy_rslt_t _cyhal_pdm_pcm_convert_decim_rate(uint16_t decim_rate, cy_en_pdm_pcm_ch_fir1_decimcode_t* pdl_code)
{
    switch(decim_rate)
    {
        case 32:
                                                             ...
                                                             cy_stc_pdm_pcm_channel_config_t* pdl_chan_config_paired)
{
    ...
    if(CY_RSLT_SUCCESS == result)
    {
        ...
        else // User-provided clock or clock already initialized by another channel, all we can do is adjust/check our internal divider
        {
            ...
            if(diff <= CLK_TOLERANCE.value && best_divider <= MAX_CLK_DIV && best_divider >= MIN_CLK_DIV)
            {
    ...
    else
    {
        ...
        if(CY_RSLT_SUCCESS == result)
        {
            ...
            const cyhal_clock_tolerance_t tolerance = {
                ...
                .value = _CYHAL_PDM_HF_CLK_TOLERANCE_HZ
            };
            /* The maximum clk frequency */
            if ((CY_RSLT_SUCCESS == result) && cyhal_clock_get_frequency(&(obj->clock)) > _CYHAL_PDM_MAX_HF_CLK_HZ)
            {
                result = cyhal_clock_set_enabled(&(obj->clock), false, true);
                if (CY_RSLT_SUCCESS == result)
                {
                    result = cyhal_clock_set_frequency(&(obj->clock), (_CYHAL_PDM_MAX_HF_CLK_HZ - _CYHAL_PDM_HF_CLK_TOLERANCE_HZ), &tolerance);
                }
                if (CY_RSLT_SUCCESS == result)
                {
                    result = cyhal_clock_set_enabled(&(obj->clock), true, true);
