...
*
* \section section_cat1d_lptimer_usage LPTimer on CAT1D
* For CAT1D devices, 2 instances of LPTimer are available, however the first one is reserved for usage on 
* the CM33 core while the second one is reserved for usage on the CM55 core. 
* This is due to the fact that interrupts related to LPTimer, MCWDT0 and MCWDT1, are only available
* respectively on CM33 and CM55. 
* HAL therefore only allows each core to initialize one LPTimer instance per core and will automatically
...
static const cy_stc_mcwdt_config_t _DEFAULT_CFG = {
                .c0UpperLimit = 0xFFFF,
                .c0WarnLimit =  0xFFFF,
...
static const cy_stc_wdc_config_t _DEFAULT_CFG = {
...
cy_rslt_t cyhal_lptimer_init(cyhal_lptimer_t *obj)
{
    ...
    if (CY_RSLT_SUCCESS == rslt)
    {
        ...
        cy_stc_mcwdt_config_t cfg = _DEFAULT_CFG;
#elif defined (_CYHAL_LPTIMER_MCWDT)
        cy_stc_mcwdt_config_t cfg = _DEFAULT_CFG;
#elif defined (CY_IP_M0S8WCO)
        cy_stc_wdc_config_t cfg = _DEFAULT_CFG;
