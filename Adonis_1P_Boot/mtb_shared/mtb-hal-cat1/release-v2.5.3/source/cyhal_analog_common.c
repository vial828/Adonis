...
#endif
#if (CY_IP_MXS40PASS) || (CY_IP_MXS22LPPASS)
static uint16_t cyhal_analog_ref_count = 0;
#endif // (CY_IP_MXS40PASS) || (CY_IP_MXS22LPPASS)
...
cy_rslt_t _cyhal_opamp_init_common(cyhal_resource_inst_t* rsc, cy_rslt_t bad_arg_error, cyhal_gpio_t vin_p, cyhal_gpio_t vin_m, cyhal_gpio_t vout, cyhal_gpio_t comp_out)
{
    ...
    const cyhal_resource_pin_mapping_t *vin_m_map = NULL;
    if (NC != vin_m)
    {
#if defined(CYHAL_PIN_MAP_DRIVE_MODE_OPAMP_VIN_M)
        vin_m_map = _CYHAL_UTILS_GET_RESOURCE(vin_m, cyhal_pin_map_opamp_vin_m);
#elif defined(CYHAL_PIN_MAP_DRIVE_MODE_OPAMP_VIN_M0)
        vin_m_map = _CYHAL_UTILS_GET_RESOURCE(vin_m, cyhal_pin_map_opamp_vin_m0);
#if defined(CYHAL_PIN_MAP_DRIVE_MODE_OPAMP_VIN_M1)
        if (NULL == vin_m_map)
        {
            vin_m_map = _CYHAL_UTILS_GET_RESOURCE(vin_m, cyhal_pin_map_opamp_vin_m1);
        }
#endif // CYHAL_PIN_MAP_DRIVE_MODE_OPAMP_VIN_M1
#endif // CYHAL_PIN_MAP_DRIVE_MODE_OPAMP_VIN_M or CYHAL_PIN_MAP_DRIVE_MODE_OPAMP_VIN_M0
    }

    const cyhal_resource_pin_mapping_t *vout_map  = (NC != vout) ?
            _CYHAL_UTILS_GET_RESOURCE(vout, cyhal_pin_map_opamp_out_10x) : NULL;
    const cyhal_resource_pin_mapping_t *comp_map  =
#ifdef CYHAL_PIN_MAP_DRIVE_MODE_OPAMP_DSI_CTB_CMP
    (NC != comp_out) ? _CYHAL_UTILS_GET_RESOURCE(comp_out, cyhal_pin_map_opamp_dsi_ctb_cmp) : NULL;
#else
    NULL;
#endif
    ...
    if (NULL != vin_p_map)
    {
        ...
        if(CY_RSLT_SUCCESS == result)
        {
            ...
            if(NC != vin_m)
            {
                result = _cyhal_utils_reserve_and_connect(vin_m_map,
                #ifdef CYHAL_PIN_MAP_DRIVE_MODE_OPAMP_VIN_M
                    CYHAL_PIN_MAP_DRIVE_MODE_OPAMP_VIN_M
                #else
                    CYHAL_PIN_MAP_DRIVE_MODE_OPAMP_VIN_M0
                #endif
                );
            }
        }
...
#endif /* (_CYHAL_DRIVER_AVAILABLE_PASS) || (_CYHAL_DRIVER_AVAILABLE_LPPASS) */
