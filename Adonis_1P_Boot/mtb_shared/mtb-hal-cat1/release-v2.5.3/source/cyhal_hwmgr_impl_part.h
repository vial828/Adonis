...
#define CY_CHANNEL_COUNT_OPAMP      (2 * CY_BLOCK_COUNT_OPAMP)
    ...
    #define CY_BLOCK_COUNT_PTC      (CY_IP_MXS22LPPASS_PTC_INSTANCES)
#else
    #define CY_BLOCK_COUNT_PTC      (0)
#endif
#define CY_CHANNEL_COUNT_PTC        (2 * CY_BLOCK_COUNT_PTC)
...
#define CY_OFFSET_PTC      (CY_OFFSET_PDMPCM + CY_SIZE_PDMPCM)
#define CY_SIZE_PTC        CY_CHANNEL_COUNT_PTC
#define CY_OFFSET_QSPI     (CY_OFFSET_PTC + CY_SIZE_PTC)
...
#endif
/* The order of items here must match the order in cyhal_hw_resources.h's cyhal_clock_block_t enum.
 *
...
static const _cyhal_hwmgr_offset_t cyhal_block_offsets_ptc[] =
{
    0,
#if defined(CY_IP_MXS22LPPASS_PTC_INSTANCES)
#if (CY_IP_MXS22LPPASS_PTC_INSTANCES > 1)
    #error "Unhanlded PTC count"
#endif
#endif
};
#endif
...
static inline const _cyhal_hwmgr_offset_t* _cyhal_get_block_offsets(cyhal_resource_t type)
{
    switch (type)
    {
        ...
        case CYHAL_RSC_PTC:
            return cyhal_block_offsets_ptc;
        case CYHAL_RSC_TDM:
...
static inline uint8_t _cyhal_get_block_offset_length(cyhal_resource_t type)
{
    switch (type)
    {
        ...
        case CYHAL_RSC_PTC:
            return sizeof(cyhal_block_offsets_ptc)/sizeof(cyhal_block_offsets_ptc[0]);
#endif
        case CYHAL_RSC_GPIO:
