...
uint8_t _cyhal_utils_get_peri_group(const cyhal_resource_inst_t *clocked_item)
{
    switch (clocked_item->type)
    {
        ...
        case CYHAL_RSC_I3C:
            /* Peri instance num + Peri group num */
            return _CYHAL_UTILS_PACK_INSTANCE_GROUP(0, 9);
        case CYHAL_RSC_TDM:
        case CYHAL_RSC_PDM:
            /* Peri instance num + Peri group num */
        ...
        case CYHAL_RSC_ETH:
            /* Peri instance num + Peri group num */
            return _CYHAL_UTILS_PACK_INSTANCE_GROUP(1, 2);
        case CYHAL_RSC_SDHC:
            /* Peri instance num + Peri group num */
            if (clocked_item->block_num == 1)
            {
                return _CYHAL_UTILS_PACK_INSTANCE_GROUP(1, 3);
            }
            else
            {
                return _CYHAL_UTILS_PACK_INSTANCE_GROUP(1, 2);
...
cy_rslt_t _cyhal_utils_allocate_clock(cyhal_clock_t *clock, const cyhal_resource_inst_t *clocked_item, cyhal_clock_block_t div, bool accept_larger)
{
    ...
    #if defined(COMPONENT_CAT1D)
    if (clocked_item->type == CYHAL_RSC_SMIF)
    {
        cyhal_clock_t clock_rsc = (clocked_item->block_num == 0) ? CYHAL_CLOCK_HF[3] : CYHAL_CLOCK_HF[4];
        return cyhal_clock_reserve(clock, &clock_rsc);
    }
    else
...
cy_rslt_t _cyhal_utils_set_clock_frequency(cyhal_clock_t* clock, uint32_t hz, const cyhal_clock_tolerance_t *tolerance)
{
    ...
    if(clock->block == CYHAL_CLOCK_BLOCK_HF)
    {
        ...
        if (CY_RSLT_SUCCESS == _cyhal_utils_find_hf_clk_div(source_hz, hz, tolerance, false, &divider, 0))
        {
...
cy_rslt_t _cyhal_utils_find_hf_clk_div(uint32_t hz_src, uint32_t desired_hz, const cyhal_clock_tolerance_t *tolerance,
                        bool only_below_desired, uint32_t *div, uint32_t min_hf_freq)
{
    ...
    for(uint8_t i = 0; i < sizeof(HFCLK_DIVIDERS) / sizeof(HFCLK_DIVIDERS[0]); ++i)
    {
        ...
        if (((actual_freq > desired_hz) && only_below_desired) || ((min_hf_freq != 0) && (actual_freq < min_hf_freq)))
            continue;
        ...
        if (only_below_desired || (achieved_tolerance == 0))
        {
            /* We are going from smallest divider, to highest. If we've not achieved better tolerance in
            *   this iteration, we will not achieve it in further for sure. */
            break;
        }
    }
    return retval;
...
cy_rslt_t _cyhal_utils_find_hf_source_n_divider(cyhal_clock_t *clock, uint32_t hz,
        ...
        uint32_t *div, uint32_t min_hf_freq)
{
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        if (CY_RSLT_SUCCESS == cyhal_clock_get(&temp_clock, sources[i]))
        {
            ...
            if ((0 == cur_hf_source_freq) ||
                ((min_hf_freq != 0) && (cur_hf_source_freq < min_hf_freq)) ||
            ...
            if (CYHAL_CLOCK_BLOCK_PATHMUX == temp_clock.block)
            {
                ...
                #else
                    /* CAT1D device does not have FLL clock, therefore indexes are little different */
                    || ((sources[i]->channel_num < SRSS_NUM_PLL) &&
                #endif // !defined(COMPONENT_CAT1D) or other
            ...
            if (CY_RSLT_SUCCESS == div_find_func(cur_hf_source_freq, hz, NULL, true, &cur_clock_divider, min_hf_freq))
            {
...
void _cyhal_utils_enable_ip_block(cyhal_resource_inst_t *rsc)
{
#if defined(COMPONENT_CAT1D)
    uint32_t periNum, groupNum, slaveNum, clkHfNum;
    switch (rsc->type)
    {
        case CYHAL_RSC_TDM:
            periNum  = CY_MMIO_TDM0_PERI_NR;
            groupNum = CY_MMIO_TDM0_GROUP_NR;
            slaveNum = CY_MMIO_TDM0_SLAVE_NR;
            clkHfNum = CY_MMIO_TDM0_CLK_HF_NR;
            break;
        case CYHAL_RSC_PDM:
            periNum  = CY_MMIO_PDM0_PERI_NR;
            groupNum = CY_MMIO_PDM0_GROUP_NR;
            slaveNum = CY_MMIO_PDM0_SLAVE_NR;
            clkHfNum = CY_MMIO_PDM0_CLK_HF_NR;
            break;
        case CYHAL_RSC_I3C:
            periNum  = CY_MMIO_I3C_PERI_NR;
            groupNum = CY_MMIO_I3C_GROUP_NR;
            slaveNum = CY_MMIO_I3C_SLAVE_NR;
            clkHfNum = CY_MMIO_I3C_CLK_HF_NR;
            break;
        case CYHAL_RSC_SMIF:
            #if defined(COMPONENT_CM33)
            // Enabling PD1 can only be performed from the CM33
            Cy_System_EnablePD1();
            #endif
            periNum  = CY_MMIO_SMIF0_PERI_NR;
            groupNum = CY_MMIO_SMIF0_GROUP_NR;
            slaveNum = CY_MMIO_SMIF0_SLAVE_NR;
            clkHfNum = CY_MMIO_SMIF0_CLK_HF_NR;
            break;
        case CYHAL_RSC_SDIODEV:
            periNum  = CY_MMIO_SDHC0_PERI_NR;
            groupNum = CY_MMIO_SDHC0_GROUP_NR;
            slaveNum = CY_MMIO_SDHC0_SLAVE_NR;
            clkHfNum = CY_MMIO_SDHC0_CLK_HF_NR;
            break;
        case CYHAL_RSC_SDHC:
            switch (rsc->block_num)
            {
                case 0:
                    periNum  = CY_MMIO_SDHC0_PERI_NR;
                    groupNum = CY_MMIO_SDHC0_GROUP_NR;
                    ...
                    break;
                case 1:
                    periNum  = CY_MMIO_SDHC1_PERI_NR;
                    groupNum = CY_MMIO_SDHC1_GROUP_NR;
                    slaveNum = CY_MMIO_SDHC1_SLAVE_NR;
                    clkHfNum = CY_MMIO_SDHC1_CLK_HF_NR;
                    break;
                default:
                    //Should not get here
                    CY_ASSERT(false);
            ...
            break;
        case CYHAL_RSC_GPIO:
            periNum  = CY_MMIO_GPIO_PERI_NR;
            groupNum = CY_MMIO_GPIO_GROUP_NR;
            slaveNum = CY_MMIO_GPIO_SLAVE_NR;
            clkHfNum = CY_MMIO_GPIO_CLK_HF_NR;
            Cy_SysClk_PeriGroupSlaveInit(CY_MMIO_HSIOM_PERI_NR, CY_MMIO_HSIOM_GROUP_NR, CY_MMIO_HSIOM_SLAVE_NR, CY_MMIO_HSIOM_CLK_HF_NR);
            break;
        case CYHAL_RSC_LPTIMER:
        //peri0_group0 access and BACKUP22 enabled state locked in both secure and non-secure. 
        //It can be accessed only in PC=1.
        case CYHAL_RSC_RTC:
            return;
        case CYHAL_RSC_ETH:
            periNum  = CY_MMIO_ETH0_PERI_NR;
            groupNum = CY_MMIO_ETH0_GROUP_NR;
            slaveNum = CY_MMIO_ETH0_SLAVE_NR;
            clkHfNum = CY_MMIO_ETH0_CLK_HF_NR;
            break;    
        case CYHAL_RSC_TCPWM:
            periNum  = CY_MMIO_TCPWM0_PERI_NR;
            groupNum = CY_MMIO_TCPWM0_GROUP_NR;
            slaveNum = CY_MMIO_TCPWM0_SLAVE_NR;
            clkHfNum = CY_MMIO_TCPWM0_CLK_HF_NR;
            break; 
        case CYHAL_RSC_LPCOMP:
            periNum  = CY_MMIO_LPCOMP_PERI_NR;
            groupNum = CY_MMIO_LPCOMP_GROUP_NR;
            slaveNum = CY_MMIO_LPCOMP_SLAVE_NR;
            clkHfNum = CY_MMIO_LPCOMP_CLK_HF_NR;
            break; 
        case CYHAL_RSC_SCB:
            switch (rsc->block_num)
            {
                case 0:
                    periNum  = CY_MMIO_SCB0_PERI_NR;
                    groupNum = CY_MMIO_SCB0_GROUP_NR;
                    slaveNum = CY_MMIO_SCB0_SLAVE_NR;
                    clkHfNum = CY_MMIO_SCB0_CLK_HF_NR;
                    break;
                case 1:
                    periNum  = CY_MMIO_SCB1_PERI_NR;
                    groupNum = CY_MMIO_SCB1_GROUP_NR;
                    slaveNum = CY_MMIO_SCB1_SLAVE_NR;
                    clkHfNum = CY_MMIO_SCB1_CLK_HF_NR;
                    break;
                case 2:
                    periNum  = CY_MMIO_SCB2_PERI_NR;
                    groupNum = CY_MMIO_SCB2_GROUP_NR;
                    slaveNum = CY_MMIO_SCB2_SLAVE_NR;
                    clkHfNum = CY_MMIO_SCB2_CLK_HF_NR;
                    break;
                case 3:
                    periNum  = CY_MMIO_SCB3_PERI_NR;
                    groupNum = CY_MMIO_SCB3_GROUP_NR;
                    slaveNum = CY_MMIO_SCB3_SLAVE_NR;
                    clkHfNum = CY_MMIO_SCB3_CLK_HF_NR;
                    break;
                case 4:
                    periNum  = CY_MMIO_SCB4_PERI_NR;
                    groupNum = CY_MMIO_SCB4_GROUP_NR;
                    slaveNum = CY_MMIO_SCB4_SLAVE_NR;
                    clkHfNum = CY_MMIO_SCB4_CLK_HF_NR;
                    break;
                case 5:
                    periNum  = CY_MMIO_SCB5_PERI_NR;
                    groupNum = CY_MMIO_SCB5_GROUP_NR;
                    slaveNum = CY_MMIO_SCB5_SLAVE_NR;
                    clkHfNum = CY_MMIO_SCB5_CLK_HF_NR;
                    break;
                case 6:
                    periNum  = CY_MMIO_SCB6_PERI_NR;
                    groupNum = CY_MMIO_SCB6_GROUP_NR;
                    slaveNum = CY_MMIO_SCB6_SLAVE_NR;
                    clkHfNum = CY_MMIO_SCB6_CLK_HF_NR;
                    break;
                case 7:
                    periNum  = CY_MMIO_SCB7_PERI_NR;
                    groupNum = CY_MMIO_SCB7_GROUP_NR;
                    slaveNum = CY_MMIO_SCB7_SLAVE_NR;
                    clkHfNum = CY_MMIO_SCB7_CLK_HF_NR;
                    break;
                case 8:
                    periNum  = CY_MMIO_SCB8_PERI_NR;
                    groupNum = CY_MMIO_SCB8_GROUP_NR;
                    slaveNum = CY_MMIO_SCB8_SLAVE_NR;
                    clkHfNum = CY_MMIO_SCB8_CLK_HF_NR;
                    break;
                case 9:
                    periNum  = CY_MMIO_SCB9_PERI_NR;
                    groupNum = CY_MMIO_SCB9_GROUP_NR;
                    slaveNum = CY_MMIO_SCB9_SLAVE_NR;
                    clkHfNum = CY_MMIO_SCB9_CLK_HF_NR;
                    break;
                case 10:
                    periNum  = CY_MMIO_SCB10_PERI_NR;
                    groupNum = CY_MMIO_SCB10_GROUP_NR;
                    slaveNum = CY_MMIO_SCB10_SLAVE_NR;
                    clkHfNum = CY_MMIO_SCB10_CLK_HF_NR;
                    break;
                case 11:
                    periNum  = CY_MMIO_SCB11_PERI_NR;
                    groupNum = CY_MMIO_SCB11_GROUP_NR;
                    slaveNum = CY_MMIO_SCB11_SLAVE_NR;
                    clkHfNum = CY_MMIO_SCB11_CLK_HF_NR;
                    break;
                default:
                    //Should not get here
                    CY_ASSERT(false);
    ...
    Cy_SysClk_PeriGroupSlaveInit(periNum, groupNum, slaveNum, clkHfNum);
#else
    CY_UNUSED_PARAMETER(rsc);
#endif
}

uint32_t _cyhal_utils_calculate_max_clk_div(cyhal_clock_block_t block)
{
    uint32_t bits;
    uint32_t max_div;

    if ((block & 0x02) == 0) // Integer (8 or 16)
    {
        bits = (block & 0x01) ? 16 : 8;
    }
    else //Fractional (16.5 or 24.5)
    {
        bits = (block & 0x01) ? 29 : 21; // Integer bits + 5
    }
    
    max_div = (((uint32_t) 1) << bits);
