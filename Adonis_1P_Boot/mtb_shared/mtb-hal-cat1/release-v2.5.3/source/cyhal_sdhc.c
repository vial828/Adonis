...
#define _CYHAL_SDHC_EXPECTED_BASE_CLK_FREQ_HZ           (_CYHAL_UTILS_MHZ_TO_HZ(100))
...
#define _CYHAL_SDHC_EMMC_MAX_SUP_FREQ_HZ                (_CYHAL_UTILS_MHZ_TO_HZ(52))
...
#define _CYHAL_SDIO_CLK_HF_HZ                           (_CYHAL_UTILS_MHZ_TO_HZ(100))
...
static const en_clk_dst_t _CYHAL_SDXX_CLK_DEST[2] = {PCLK_SDHC0_CLK_HF, PCLK_SDHC1_CLK_HF};
#endif
...
/***********************************************************************************************************************
*
* Finds SDHC internal divider value according to source clock frequency and desired frequency for a peri clock.
* Allows for divider to be an odd value as opposed to even required by _cyhal_sdxx_find_best_div.
*
* hz_src                - Source clock frequency, that needs to be divided
* desired_hz            - Desired clock frequency
* tolerance             - (Not used)
...
*
***********************************************************************************************************************/
static cy_rslt_t _cyhal_sdxx_find_best_div_peri(uint32_t hz_src, uint32_t desired_hz,
            const cyhal_clock_tolerance_t *tolerance, bool only_below_desired, uint32_t *div, uint32_t min_hf_freq)
{
    CY_UNUSED_PARAMETER(tolerance);
    CY_UNUSED_PARAMETER(only_below_desired);
    ...
    */
    ...
    if (hz_src > desired_hz)
    {
        uint32_t freq = (desired_hz);
        uint32_t calculated_divider = ((hz_src + freq - 1) / freq) & 0x3FF;
        *div = calculated_divider;
    }
    else
    {
        *div = 1;
...
static cy_rslt_t _cyhal_sdxx_update_hw_clock_config(_cyhal_sdxx_t *sdxx, uint32_t freq, bool lv_signaling, bool negotiate)
{
    ...
    if (!sdxx->is_sdio && sdxx->emmc)
    {
        ...
        if (freq <= _CYHAL_UTILS_MHZ_TO_HZ(26))
        {
...
static cy_rslt_t _cyhal_sdxx_sdcardchangeclock(_cyhal_sdxx_t *sdxx, uint32_t *frequency, bool lv_signaling, bool negotiate)
{
    ...
    uint32_t most_suitable_peri_div = 0;
    #endif
    cy_rslt_t ret = CY_RSLT_SUCCESS;

    #if defined(CY_IP_MXS22SRSS)
    if(*frequency > _CYHAL_UTILS_MHZ_TO_HZ(100))
    {
        ret = CYHAL_SDHC_RSLT_ERR_CLOCK;
    }
    #endif
    ...
    if (CY_RSLT_SUCCESS == ret)
    {
        if (sdxx->clock_owned)
        {
            if (sdxx->clock.block == CYHAL_CLOCK_BLOCK_HF)
            {
                ...
                *   No clock settings are being changed here. */
                ret = _cyhal_utils_find_hf_source_n_divider(&(sdxx->clock), *frequency, NULL, _cyhal_sdxx_find_best_div,
                                    &most_suitable_hf_source, &most_suitable_div, 0);
            }
            else
            {
                ret = _cyhal_utils_find_hf_source_n_divider(&(sdxx->clock), *frequency, NULL, _cyhal_sdxx_find_best_div_peri,
                                        &most_suitable_hf_source, &most_suitable_div, 0);
            }
        }
        else
        {
            ret = _cyhal_sdxx_find_best_div(cyhal_clock_get_frequency(&(sdxx->clock)), *frequency, NULL, false, &most_suitable_div, 0);
        }
    }
    ...
    if (CY_RSLT_SUCCESS == ret)
    {
        ...
        bus_freq = hf_freq / most_suitable_div;
        #else
        if (most_suitable_div <= 1) //peripheral clock divider needs to be at least two.
        {
            ret = CYHAL_SDHC_RSLT_ERR_CLOCK;
        }
        ...
        if (CY_RSLT_SUCCESS == ret)
        {
            if (sdxx->clock_owned)
            {
                hf_freq = cyhal_clock_get_frequency(&most_suitable_hf_source);

                most_suitable_peri_div = 2; // Lowest peri div for PSE84 EVK
                uint32_t max_div = _cyhal_utils_calculate_max_clk_div(sdxx->clock.block);
                uint32_t diff;
                uint32_t best_diff = hf_freq;
                uint32_t best_div = 1;
                uint32_t best_peri_div = 2;
                uint32_t calculated_final_freq = hf_freq/best_peri_div;

                while(most_suitable_peri_div <= max_div)
                {
                    _cyhal_sdxx_find_best_div_peri(hf_freq/most_suitable_peri_div, *frequency, NULL, NULL, &most_suitable_div, 0);
                    calculated_final_freq = hf_freq/most_suitable_peri_div/most_suitable_div;
                    diff = calculated_final_freq > *frequency ? calculated_final_freq - *frequency : *frequency - calculated_final_freq;

                    if (diff < best_diff)
                    {
                        best_diff = diff;
                        best_div = most_suitable_div;
                        best_peri_div = most_suitable_peri_div;
                        if(best_diff == 0)
                        {
                            break;
                        }
                    }
                    most_suitable_peri_div++;
                }
                most_suitable_div = best_div;
                most_suitable_peri_div = best_peri_div;

                ret = cyhal_clock_set_divider(&(sdxx->clock), most_suitable_peri_div);

                peri_freq = cyhal_clock_get_frequency(&(sdxx->clock));
                bus_freq = peri_freq / most_suitable_div;
            }
            else
            {
                bus_freq = hf_freq / most_suitable_div;
        ...
        if (CY_RSLT_SUCCESS == ret)
        {
            ret = _cyhal_sdxx_update_hw_clock_config(sdxx, bus_freq, lv_signaling, negotiate);
        }
    }
    ...
    if (!sdxx->is_sdio)
    {
        ...
        #else
        ((cyhal_sdhc_t *) sdxx->obj)->block_source_freq_hz = peri_freq;
        #endif
    }
...
static cy_rslt_t _cyhal_sdhc_init_common_hw(cyhal_sdhc_t *obj, const cyhal_sdhc_configurator_t *cfg)
{
    ...
    uint8_t i = 0;
    while ((i < 4) && (CY_RSLT_SUCCESS == result))
    {
        if (NC != data[i])
        {
    ...
    i = 4;
    while ((i < 8) && (CY_RSLT_SUCCESS == result))
    {
        if (NC != data[i])
        {
    ...
    {
        #if defined(CY_IP_MXS22SRSS)
        _cyhal_utils_enable_ip_block(&rsc);
        #endif
        ...
        if (cfg->clock == NULL)
        {
            ...
            #if defined (CY_IP_MXS22SRSS)
            result = _cyhal_utils_peri_pclk_assign_divider(_CYHAL_SDXX_CLK_DEST[rsc.block_num], &(sdxx->clock));
            #endif

            if (CY_RSLT_SUCCESS == result)
            {
                sdxx->clock_owned = true;
                result = _cyhal_utils_set_clock_frequency(&(sdxx->clock), MAX_FREQUENCY, &CYHAL_CLOCK_TOLERANCE_5_P);
            }
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        ...
        *   for passthrough (to be same as SDHC block source clock) */
        result = cyhal_sdhc_set_frequency(obj,
                        sdxx->clock_owned ? _CYHAL_UTILS_MHZ_TO_HZ(25) : cyhal_clock_get_frequency(&(sdxx->clock)), true);
    }
...
static cy_rslt_t _cyhal_sdio_init_common(cyhal_sdio_t *obj, const cyhal_sdio_configurator_t *cfg)
{
    ...
    if (result == CY_RSLT_SUCCESS)
    {
        ...
        #if defined(CY_IP_MXS22SRSS)
        _cyhal_utils_enable_ip_block(&sdhc);
        #endif

        if (cfg->clock == NULL)
        {
            #if defined (CY_IP_MXS22SRSS)
            result = _cyhal_utils_allocate_clock(&(sdxx->clock), &sdhc, CYHAL_CLOCK_BLOCK_PERIPHERAL_8BIT, true);
            result = _cyhal_utils_peri_pclk_assign_divider(_CYHAL_SDXX_CLK_DEST[sdhc.block_num], &(sdxx->clock));
            #else
