...
*
* On CAT1D, the comparator driver can additionally use the Programmable Threshold Comparator
* block (PTC)
*
* Generally, a set of pins can only connect to one of the above blocks. In the event
* that multiple connections are possible, the order of priority will be:
* - LPComp
* - Opamp
* - PTC
*
...
* | HAL Power Level                | Opamp Power Level   | LPComp Power Level           | PTC Power Level (CAT1D Only)        |
* | ------------------------------ | ------------------- | ---------------------------- | ----------------------------------- |
* | @ref CYHAL_POWER_LEVEL_HIGH    | CY_CTB_POWER_HIGH   | CY_LPCOMP_MODE_[NORMAL/FAST] | CY_AUTANALOG_PTCOMP_COMP_PWR_OFF    |
* | @ref CYHAL_POWER_LEVEL_MEDIUM  | CY_CTB_POWER_MEDIUM | CY_LPCOMP_MODE_[LP/SLOW]     | CY_AUTANALOG_PTCOMP_COMP_PWR_ULP    |
* | @ref CYHAL_POWER_LEVEL_LOW     | CY_CTB_POWER_LOW    | CY_LPCOMP_MODE_ULP           | CY_AUTANALOG_PTCOMP_COMP_PWR_LP     |
* | @ref CYHAL_POWER_LEVEL_DEFAULT | CY_CTB_POWER_MEDIUM | CY_LPCOMP_MODE_[LP/SLOW]     | CY_AUTANALOG_PTCOMP_COMP_PWR_NORMAL |
*
* \} group_hal_impl_comp
*/
...
#endif
#if (_CYHAL_DRIVER_AVAILABLE_COMP_PTC)
    #if ((_CYHAL_DRIVER_AVAILABLE_COMP_LP) || (_CYHAL_DRIVER_AVAILABLE_COMP_CTB))
    if (CY_RSLT_SUCCESS != result)
    #endif
    {
        result = _cyhal_comp_ptc_init(obj, vin_p, vin_m, output, cfg);
    }
#endif
    return result;
}
...
#if (_CYHAL_DRIVER_AVAILABLE_COMP_PTC)
    if (CYHAL_RSC_PTC == cfg->resource->type)
    {
        result = _cyhal_comp_ptc_init_cfg(obj, cfg);
    }
#endif
...
{
#if (_CYHAL_DRIVER_AVAILABLE_COMP_CTB)
    #if (_CYHAL_DRIVER_AVAILABLE_COMP_LP) || (_CYHAL_DRIVER_AVAILABLE_COMP_PTC)
    ...
    {
#if (_CYHAL_DRIVER_AVAILABLE_COMP_LP)
    #if (_CYHAL_DRIVER_AVAILABLE_COMP_CTB) || (_CYHAL_DRIVER_AVAILABLE_COMP_PTC)
...
#endif
#if (_CYHAL_DRIVER_AVAILABLE_COMP_PTC)
    #if (_CYHAL_DRIVER_AVAILABLE_COMP_CTB) || (_CYHAL_DRIVER_AVAILABLE_COMP_LP)
    if (obj->resource.type == CYHAL_RSC_PTC)
    #endif
    {
...
cy_rslt_t cyhal_comp_set_power(cyhal_comp_t *obj, cyhal_power_level_t power)
{
    ...
    #endif
    {
        return _cyhal_comp_ptc_set_power(obj, power);
    }
#endif
#if (_CYHAL_DRIVER_AVAILABLE_COMP_LP)
...
cy_rslt_t cyhal_comp_configure(cyhal_comp_t *obj, cyhal_comp_config_t *cfg)
{
    ...
    #endif
    {
        return _cyhal_comp_ptc_configure(obj, cfg);
    }
#endif
#if (_CYHAL_DRIVER_AVAILABLE_COMP_LP)
    CY_ASSERT(obj->resource.type == CYHAL_RSC_LPCOMP);
...
bool cyhal_comp_read(cyhal_comp_t *obj)
{
    ...
    #endif
    {
        return _cyhal_comp_ptc_read(obj);
    }
#endif
#if (_CYHAL_DRIVER_AVAILABLE_COMP_LP)
    CY_ASSERT(obj->resource.type == CYHAL_RSC_LPCOMP);
...
void cyhal_comp_enable_event(cyhal_comp_t *obj, cyhal_comp_event_t event, uint8_t intr_priority, bool enable)
{
    ...
    #endif
    {
        _cyhal_comp_ptc_enable_event(obj, event, intr_priority, enable);
        return;
    }
#endif
#if (_CYHAL_DRIVER_AVAILABLE_COMP_LP)
