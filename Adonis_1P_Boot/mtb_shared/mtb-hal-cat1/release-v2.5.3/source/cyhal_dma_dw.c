...
cy_rslt_t _cyhal_dma_dw_configure(cyhal_dma_t *obj, const cyhal_dma_cfg_t *cfg)
{
    ...
    // DataWire only supports <=CY_DMA_LOOP_COUNT_MAX (256) byte burst and <=CY_DMA_LOOP_COUNT_MAX bytes per burst
    if ((cfg->burst_size > CY_DMA_LOOP_COUNT_MAX) ||
        (cfg->burst_size <= 1 && cfg->length > CY_DMA_LOOP_COUNT_MAX) ||
        (cfg->burst_size > 0 && cfg->length > (cfg->burst_size * CY_DMA_LOOP_COUNT_MAX)))
    ...
    obj->descriptor_config.dw.srcTransferSize = CY_DMA_TRANSFER_SIZE_DATA;
...
static cy_en_dma_trigger_type_t _cyhal_dma_dw_convert_input_t(cyhal_dma_input_t input)
{
    switch(input)
    {
...
static cy_en_dma_trigger_type_t _cyhal_dma_dw_convert_output_t(cyhal_dma_output_t output)
{
    switch(output)
    {
...
cy_rslt_t _cyhal_dma_dw_connect_digital(cyhal_dma_t *obj, cyhal_source_t source, cyhal_dma_input_t input)
{
    ...
    obj->descriptor_config.dw.triggerInType = _cyhal_dma_dw_convert_input_t(input);
...
cy_rslt_t _cyhal_dma_dw_enable_output(cyhal_dma_t *obj, cyhal_dma_output_t output, cyhal_source_t *source)
{
    ...
    obj->descriptor.dw.ctl |= _VAL2FLD(CY_DMA_CTL_TR_OUT_TYPE, _cyhal_dma_dw_convert_output_t(output));
...
uint32_t _cyhal_dma_dw_get_max_elements_per_burst(cyhal_dma_t *obj)
{
