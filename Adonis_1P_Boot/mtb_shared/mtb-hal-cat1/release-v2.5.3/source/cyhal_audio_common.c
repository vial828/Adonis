...
/* The maximum HF clock should not be greater than 200 MHz according to the TRM */
#define _CYHAL_AUDIOSS_MAX_HF_CLK_HZ               (200000000u)
/* The maximum tolerance of desierd HF clock in Hz */
#define _CYHAL_AUDIOSS_HF_CLK_TOLERANCE_HZ         (25000000u)
...
#define _CYHAL_AUDIOSS_INST_NUM(resource)         ((resource).block_num)
...
#if ((CY_IP_MXTDM_VERSION) >= 3)
#define _CYHAL_AUDIOSS_TDM_BLOCK_OFFSET         (1)
#define _CYHAL_AUDIOSS_TDM_CHANNEL_OFFSET       (0)
#define _CYHAL_AUDIOSS_TDM_INSTANCES            (TDM_NR)
#else
#define _CYHAL_AUDIOSS_TDM_BLOCK_OFFSET         (0)
#define _CYHAL_AUDIOSS_TDM_CHANNEL_OFFSET       (0)
#define _CYHAL_AUDIOSS_TDM_INSTANCES            (CY_IP_MXTDM_INSTANCES)
#endif /* defined(COMPONENT_CAT1D) */

#define _CYHAL_AUDIOSS_INST_NUM(resource)         ((((resource).block_num) << (_CYHAL_AUDIOSS_TDM_BLOCK_OFFSET)) | (((resource).channel_num) << (_CYHAL_AUDIOSS_TDM_CHANNEL_OFFSET)))
...
#else
static _cyhal_audioss_t* _cyhal_audioss_config_structs[_CYHAL_AUDIOSS_TDM_INSTANCES];
#endif
...
static const _cyhal_system_irq_t _cyhal_audioss_tx_irq_n[] =
{
    ...
    #else
        #if defined(TDM0_TDM_STRUCT0)
            tdm_0_interrupts_tx_0_IRQn,
        #endif
        #if defined(TDM0_TDM_STRUCT1)
...
static uint8_t _cyhal_audioss_get_block_from_irqn(_cyhal_system_irq_t irqn)
{
    ...
    #endif
    #if defined(TDM0_TDM_STRUCT1)
        if ((irqn == tdm_0_interrupts_tx_1_IRQn) || (irqn == tdm_0_interrupts_rx_1_IRQn))
            return 1;
    #endif
#endif
#if defined (TDM1)
...
cy_rslt_t _cyhal_audioss_init_clock(_cyhal_audioss_t *obj, const cyhal_clock_t* clk, bool all_mclk)
{
    ...
    else if (false == all_mclk) // No need to reserve a clock if we're using the mclk pin
    {
        ...
        if(CY_RSLT_SUCCESS == result)
        {
            ...
            const cyhal_clock_tolerance_t tolerance = {
                .type = CYHAL_TOLERANCE_HZ,
                .value = _CYHAL_AUDIOSS_HF_CLK_TOLERANCE_HZ
            };
            /* The maximum clk frequency */
            if ((CY_RSLT_SUCCESS == result) && cyhal_clock_get_frequency(&(obj->clock)) > _CYHAL_AUDIOSS_MAX_HF_CLK_HZ)
            {
                result = cyhal_clock_set_enabled(&(obj->clock), false, true);
                if (CY_RSLT_SUCCESS == result)
                {
                    result = cyhal_clock_set_frequency(&(obj->clock), (_CYHAL_AUDIOSS_MAX_HF_CLK_HZ - _CYHAL_AUDIOSS_HF_CLK_TOLERANCE_HZ), &tolerance);
                }
                if (CY_RSLT_SUCCESS == result)
                {
                    result = cyhal_clock_set_enabled(&(obj->clock), true, true);
    ...
    {
        uint8_t instance = _CYHAL_AUDIOSS_INST_NUM(obj->resource);
        en_clk_dst_t pclk = _cyhal_audioss_clock[instance];
...
{
    uint8_t tdm_inst = _CYHAL_AUDIOSS_INST_NUM(obj->resource);
#if defined(CY_IP_MXAUDIOSS)
...
#elif defined(CY_IP_MXTDM)
        uint8_t instance = _CYHAL_AUDIOSS_INST_NUM(obj->resource);
        _cyhal_system_irq_t irqn = _cyhal_audioss_rx_irq_n[instance];
        _cyhal_irq_free(irqn);
        irqn = _cyhal_audioss_tx_irq_n[instance];
        _cyhal_irq_free(irqn);
#endif
...
#elif defined(CY_IP_MXTDM)
    uint8_t instance = _CYHAL_AUDIOSS_INST_NUM(obj->resource);
    _cyhal_system_irq_t irqn = _cyhal_audioss_tx_irq_n[instance];
    _cyhal_irq_set_priority(irqn, intr_priority);
    irqn = _cyhal_audioss_rx_irq_n[instance];
    _cyhal_irq_set_priority(irqn, intr_priority);
#endif
}
                                                    ...
                                                    bool rx_en, bool tx_en, bool mclk_rx, bool mclk_tx)
{
    ...
    if(CY_RSLT_SUCCESS == result)
    {
        ...
        pdl_config->tx_config->clkSel = mclk_tx ? CY_TDM_SEL_MCLK_IN : (cy_en_tdm_clock_sel_t)_CYHAL_AUDIOSS_INST_NUM(obj->resource);
        ...
        pdl_config->rx_config->clkDiv = sclk_div_rx;
        /* Only clock 0 from the srss hooked up on current hardware */
        pdl_config->rx_config->clkSel = mclk_rx ? CY_TDM_SEL_MCLK_IN : (cy_en_tdm_clock_sel_t)_CYHAL_AUDIOSS_INST_NUM(obj->resource);
...
static cyhal_source_t _cyhal_audioss_calculate_source(_cyhal_audioss_t *obj, bool is_rx)
{
    uint8_t instance = _CYHAL_AUDIOSS_INST_NUM(obj->resource);
#if defined(_CYHAL_AUDIOSS_RX_ENABLED)
    return is_rx ? _cyhal_audioss_rx_trigger[instance] : _cyhal_audioss_tx_trigger[instance];
#else
    CY_UNUSED_PARAMETER(is_rx);
    return _cyhal_audioss_tx_trigger[instance];
#endif /* defined(_CYHAL_AUDIOSS_RX_ENABLED) */
}
#endif
