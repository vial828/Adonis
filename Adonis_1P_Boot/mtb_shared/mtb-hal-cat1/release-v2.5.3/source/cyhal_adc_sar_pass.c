...
_CYHAL_ADC_SAR_Type *const _cyhal_adc_base[] =
{
...
const en_clk_dst_t _cyhal_adc_clock[] =
{
#if defined(_CYHAL_ADC_SINGLE_UNSUFFIXED)
    PCLK_PASS_CLOCK_SAR,
...
const cyhal_dest_t _cyhal_adc_tr_in[] =
{
#if defined(_CYHAL_ADC_SINGLE_UNSUFFIXED)
...
#if defined(CY_IP_MXS40EPASS_ESAR_INSTANCES)
static _cyhal_system_irq_t _cyhal_adc_sar_calc_channel_irq(uint8_t adc_num, uint8_t channel_num)
{
...
#endif /* (CY_IP_MXS40EPASS_ESAR_INSTANCES) */
...
static void _cyhal_adc_sar_extract_channel_conf(cyhal_adc_t* adc, cy_stc_sar2_channel_config_t* channel_configs)
{
    for(uint8_t i = 0; i < _CYHAL_SAR_MAX_NUM_CHANNELS; ++i)
    {
...
#if !defined(CY_IP_MXS40EPASS_ESAR_INSTANCES)
static uint32_t _cyhal_adc_sar_get_mux_switch_control(cyhal_gpio_t gpio)
{
...
static uint32_t _cyhal_adc_sar_get_fw_switch_control(cyhal_gpio_t gpio, bool is_vplus)
{
...
#if defined(CY_IP_M0S8PASS4A_SAR_INSTANCES)
static uint32_t _cyhal_adc_sar_get_pin_addr(cyhal_gpio_t gpio)
{
...
#else
static uint32_t _cyhal_adc_sar_get_pin_addr(cyhal_gpio_t gpio, bool is_vplus)
{
...
#endif /* (CY_IP_M0S8PASS4A_SAR_INSTANCES) */

static cy_en_sar_ctrl_vref_sel_t _cyhal_adc_sar_convert_vref(cyhal_adc_vref_t vref)
{
    switch(vref)
    {
...
static cy_rslt_t _cyhal_adc_sar_convert_average_count(uint32_t hal_count, cy_en_sar_sample_ctrl_avg_cnt_t* pdl_count)
{
    switch(hal_count)
    {
...
#if defined(CY_IP_M0S8PASS4A_SAR_INSTANCES)
static cy_rslt_t _cyhal_adc_sar_convert_resolution(uint8_t hal_resolution, cy_en_sar_sample_ctrl_sub_resolution_t *pdl_resolution)
{
...
static void _cyhal_adc_sar_extract_channel_conf(cyhal_adc_t* adc, cy_stc_sar_channel_config_t* channel_configs)
{
...
static cy_rslt_t _cyhal_adc_sar_populate_pdl_config(const cyhal_adc_config_t* hal_config, cy_stc_sar_config_t* pdl_config,
                                                cy_stc_sar_channel_config_t* channel_configs /* Array, length CY_SAR_SEQ_NUM_CHANNELS */)
{
    ...
    pdl_config->vrefSel = _cyhal_adc_sar_convert_vref(hal_config->vref);
    ...
    {
        result = _cyhal_adc_sar_convert_resolution(hal_config->resolution, &pdl_config->subResolution);
    }
    if(CY_RSLT_SUCCESS == result)
    {
        ...
        result = _cyhal_adc_sar_convert_average_count(hal_config->average_count, &pdl_config->avgCnt);
    }
    if(CY_RSLT_SUCCESS == result)
    {
        ...
        pdl_config->sampleTime0 = 10u;
        pdl_config->sampleTime1 = 10u;
        pdl_config->sampleTime2 = 10u;
        pdl_config->sampleTime3 = 10u;
        ...
        pdl_config->vrefMvValue = _cyhal_adc_sar_get_vref_mv(hal_config);
    }
    return result;
}
...
static uint32_t _cyhal_adc_sar_convert_average_mode(uint32_t average_mode_flags)
{
    uint32_t result = 0;
...
static cy_rslt_t _cyhal_adc_sar_populate_pdl_config(const cyhal_adc_config_t* hal_config, cy_stc_sar_config_t* pdl_config)
{
    ...
    *pdl_config = _CYHAL_ADC_DEFAULT_PDL_CONFIG;
    ...
    ctrl |= (uint32_t)_cyhal_adc_sar_convert_vref(hal_config->vref);
    ...
    if(CY_RSLT_SUCCESS == result)
    {
        ...
        sample_ctrl |= _cyhal_adc_sar_convert_average_mode(hal_config->average_mode_flags);
...
static cy_rslt_t _cyhal_adc_sar_apply_channel_configs(cyhal_adc_t* obj, cy_stc_sar2_channel_config_t* channel_configs)
{
    ...
    for(size_t i = 0; i < _CYHAL_SAR_MAX_NUM_CHANNELS; ++i)
    {
...
static void _cyhal_adc_sar_channel_update_config(cyhal_adc_channel_t* obj, cy_stc_sar2_channel_config_t *pdl_cfg, const cyhal_adc_channel_config_t *cfg)
{
     ...
     * combined with the ADC config; that is taken care of in _cyhal_adc_sar_apply_channel_configs */
}
...
{
    *min_acquisition_ns = 0;
    *conversion_clock_cycles = 0;
    for(uint8_t i = 0; i < _CYHAL_SAR_MAX_NUM_CHANNELS; ++i)
    {
...
#endif
        {
            ...
            uint16_t average_count = _cyhal_adc_sar_get_average_count(obj, i);
...
void _cyhal_adc_sar_start_convert(cyhal_adc_t* obj)
{
    ...
    Cy_SAR2_Channel_SoftwareTrigger(obj->base, _cyhal_adc_sar_first_enabled(obj));
    #else
...
{
    #if defined(CY_IP_MXS40EPASS_ESAR_INSTANCES)
    uint8_t channel_idx = _cyhal_adc_sar_last_enabled(obj);
...
void _cyhal_adc_sar_update_interrupt_priority(cyhal_adc_t *obj, uint8_t intr_priority)
{
    ...
    for(uint8_t i = 0; i < _CYHAL_SAR_MAX_NUM_CHANNELS; ++i)
    {
        _cyhal_system_irq_t irqn = _cyhal_adc_sar_calc_channel_irq(obj->resource.block_num, i);
...
cy_rslt_t _cyhal_adc_sar_init(cyhal_adc_t *obj, cyhal_gpio_t pin, const cyhal_clock_t *clk)
{
    ...
    result =  _cyhal_adc_sar_populate_pdl_config(&_CYHAL_ADC_DEFAULT_CONFIG, &pdl_config, chan_configs);
#else
    result = _cyhal_adc_sar_populate_pdl_config(&_CYHAL_ADC_DEFAULT_CONFIG, &pdl_config);
#endif
#endif
...
cy_rslt_t _cyhal_adc_sar_init_hw(cyhal_adc_t *obj, const cyhal_adc_configurator_t* cfg)
{
    ...
    if (result == CY_RSLT_SUCCESS)
    {
        ...
        for(uint8_t i = 0; i < _CYHAL_SAR_MAX_NUM_CHANNELS; ++i)
        {
            _cyhal_irq_register(_cyhal_adc_sar_calc_channel_irq(obj->resource.block_num, i),
                                CYHAL_ISR_PRIORITY_DEFAULT, _cyhal_adc_irq_handler);
            _cyhal_irq_enable(_cyhal_adc_sar_calc_channel_irq(obj->resource.block_num, i));
        }
#else
...
void _cyhal_adc_sar_deinit(cyhal_adc_t *obj)
{
    ...
    for(uint8_t i = 0; i < _CYHAL_SAR_MAX_NUM_CHANNELS; ++i)
    {
        _cyhal_irq_free(_cyhal_adc_sar_calc_channel_irq(obj->resource.block_num, i));
    }
#else
    _cyhal_system_irq_t irqn = _cyhal_adc_irq_n[obj->resource.block_num];
...
cy_rslt_t _cyhal_adc_sar_configure(cyhal_adc_t *obj, const cyhal_adc_config_t *config)
{
    ...
    if(CY_RSLT_SUCCESS == result)
    {
        ...
        cy_stc_sar_channel_config_t chan_configs[CY_SAR_SEQ_NUM_CHANNELS];
        _cyhal_adc_sar_extract_channel_conf(obj, chan_configs);
        result = _cyhal_adc_sar_populate_pdl_config(config, &pdl_config, chan_configs);
    }
#else
    if(CY_RSLT_SUCCESS == result)
    {
        result = _cyhal_adc_sar_populate_pdl_config(config, &pdl_config);
    }
#endif
    ...
    if(CY_RSLT_SUCCESS == result)
    {
        ...
        _cyhal_adc_sar_populate_acquisition_timers(obj);
    ...
    if(CY_RSLT_SUCCESS == result)
    {
        ...
        cy_stc_sar2_channel_config_t channel_configs[_CYHAL_SAR_MAX_NUM_CHANNELS];

        _cyhal_adc_sar_extract_channel_conf(obj, channel_configs);
        result = _cyhal_adc_sar_apply_channel_configs(obj, channel_configs);
    }
#endif
...
static void _cyhal_adc_sar_channel_update_config(cyhal_adc_channel_t* obj, const cyhal_adc_channel_config_t* config)
{
...
static void _cyhal_adc_sar_channel_set_pin_config(cyhal_adc_channel_t* obj, cyhal_gpio_t vplus, cyhal_gpio_t vminus)
{
        uint32_t result = 0u;
#if defined(CY_IP_M0S8PASS4A_SAR_INSTANCES)
        CY_UNUSED_PARAMETER(vminus); // On M0S8, vminus is implied by vplus
        result |= _cyhal_adc_sar_get_pin_addr(vplus);
        result |= _BOOL2FLD(SAR_CHAN_CONFIG_DIFFERENTIAL_EN, NC != vminus);
#else
        result |= _cyhal_adc_sar_get_pin_addr(vplus, true);
        if(vminus == NC)
        {
        ...
        {
            /* Differential */
            result |= _cyhal_adc_sar_get_pin_addr(vminus, false);
...
static void _cyhal_adc_sar_update_chan_offset(cyhal_adc_channel_t* obj)
{
...
cy_rslt_t _cyhal_adc_sar_channel_init(cyhal_adc_channel_t *obj, cyhal_adc_t* adc, cyhal_gpio_t vplus, cyhal_gpio_t vminus, const cyhal_adc_channel_config_t* cfg)
{
    ...
    cy_stc_sar2_channel_config_t channel_configs[_CYHAL_SAR_MAX_NUM_CHANNELS];
    _cyhal_adc_sar_extract_channel_conf(obj->adc, channel_configs);
    ...
    if (false == adc->vbg_chan_inited)
    {
        ...
        _cyhal_adc_sar_channel_update_config(obj, &channel_configs[_CYHAL_ADC_VBG_CHANNEL_IDX], cfg);

        adc->vbg_chan_inited = true;
    }
    #endif /* defined(CY_IP_MXS40EPASS_ESAR_INSTANCES) */

    _cyhal_adc_sar_channel_update_config(obj, &channel_configs[obj->channel_idx], cfg);
    result = _cyhal_adc_sar_apply_channel_configs(obj->adc, channel_configs);
#else
    CY_UNUSED_PARAMETER(adc);

    uint32_t fw_ctrl_plus = _cyhal_adc_sar_get_fw_switch_control(vplus, true);
    uint32_t mux_ctrl_plus = _cyhal_adc_sar_get_mux_switch_control(vplus);
    ...
    {
        uint32_t fw_ctrl_minus = _cyhal_adc_sar_get_fw_switch_control(vminus, false);
        uint32_t mux_ctrl_minus = _cyhal_adc_sar_get_mux_switch_control(vminus);
    ...
    _cyhal_adc_sar_channel_set_pin_config(obj, obj->vplus, obj->vminus);
...
cy_rslt_t _cyhal_adc_sar_channel_config(cyhal_adc_channel_t *obj, const cyhal_adc_channel_config_t *config)
{
    ...
    cy_stc_sar2_channel_config_t channel_configs[_CYHAL_SAR_MAX_NUM_CHANNELS];
    _cyhal_adc_sar_extract_channel_conf(obj->adc, channel_configs);
    _cyhal_adc_sar_channel_update_config(obj, &channel_configs[obj->channel_idx], config);
    return _cyhal_adc_sar_apply_channel_configs(obj->adc, channel_configs);
#else
    _cyhal_adc_sar_channel_update_config(obj, config);
    if(config->enabled)
    {
    ...
    _cyhal_adc_sar_update_chan_offset(obj);
    return _cyhal_adc_sar_populate_acquisition_timers(obj->adc);
#endif
}
...
void _cyhal_adc_sar_channel_deinit(cyhal_adc_channel_t *obj)
{
    ...
    if(false == obj->adc->owned_by_configurator)
    {
        if(NC != obj->vplus)
        {
            uint32_t fw_ctrl_plus = _cyhal_adc_sar_get_fw_switch_control(obj->vplus, true);
            uint32_t mux_ctrl_plus = _cyhal_adc_sar_get_mux_switch_control(obj->vplus);
        ...
        if(NC != obj->vminus)
        {
            uint32_t mux_ctrl_minus = _cyhal_adc_sar_get_mux_switch_control(obj->vminus);
            uint32_t fw_ctrl_minus = _cyhal_adc_sar_get_fw_switch_control(obj->vminus, false);
    ...
    if (obj->adc->vbg_chan_inited)
    {
        ...
        for(uint8_t chan_idx = 0; chan_idx < _CYHAL_SAR_MAX_NUM_CHANNELS; ++chan_idx)
        {
...
{
#if defined(CY_IP_MXS40EPASS_ESAR_INSTANCES)
    return Cy_SAR2_Channel_GetResult(obj->base, channel, NULL);
#else
    return Cy_SAR_GetResult32(obj->base, channel);
#endif
}
...
int32_t _cyhal_adc_sar_read(const cyhal_adc_channel_t *obj)
{
    ...
    if(!obj->adc->continuous_scanning)
    {
        ...
        for(uint8_t i = 0; i < _CYHAL_SAR_MAX_NUM_CHANNELS; ++i) /* Fortunately, each ADC supports a max of 32 channels */
        {
        ...
        _cyhal_adc_sar_apply_channel_configs(obj->adc, channel_configs);
#else
#if defined(CY_IP_MXS40PASS_SAR_INSTANCES)
        ...
        cy_en_sar_start_convert_sel_t mode =
            (isChannelInterleaved) ? CY_SAR_START_CONVERT_CONTINUOUS : CY_SAR_START_CONVERT_SINGLE_SHOT;
#else
        bool isChannelInterleaved = false;
        cy_en_sar_start_convert_sel_t mode = CY_SAR_START_CONVERT_SINGLE_SHOT;
#endif
...
#if defined(CY_IP_MXS40EPASS_ESAR)
        Cy_SAR2_Channel_SoftwareTrigger(obj->adc->base, _cyhal_adc_sar_first_enabled(obj->adc));
#else
        Cy_SAR_StartConvert(obj->adc->base, mode);
#endif
    }
