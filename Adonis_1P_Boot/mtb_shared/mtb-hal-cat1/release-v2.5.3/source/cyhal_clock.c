...
#if defined(TARGET_PSVP_CYW20829) || defined(COMPONENT_CAT1D)
...
/* CAT1A hasn't CY_SYSCLK_CLK_CORE_HF_PATH_NUM define and
 * use HF0 clock as a source path in SystemCoreClockUpdate() */
#ifndef CY_SYSCLK_CLK_CORE_HF_PATH_NUM
    #define CY_SYSCLK_CLK_CORE_HF_PATH_NUM      (0UL)
#endif /* CY_SYSCLK_CLK_CORE_HF_PATH_NUM */

//For fractional dividers we need to shift the bits
#define _CYHAL_CLOCK_FRAC_DIV_DIGITS 5u

//Default integer divider
#define _CYHAL_CLOCK_MAX_INT_DIV 256u

#if defined (CY_IP_MXS22SRSS) && defined(COMPONENT_CM55)
#define _CYHAL_CLOCK_CPU_HFCLK_IDX 1
#else
#define _CYHAL_CLOCK_CPU_HFCLK_IDX 0
#endif
/******************************************************************************
 ****************************** Clock Resources *******************************
 *****************************************************************************/
...
#define _CYHAL_CLOCK_ECO_MAX_INT_DIV 1024 //ECO Prescaler uses a different maximum integer divider than othwe clocks
#endif
...
static uint32_t _cyhal_clock_get_lf_frequency(void)
{
    ...
    switch (source)
    {
        ...
        case CY_SYSCLK_CLKLF_IN_ECO_PRESCALER:
            return Cy_SysClk_EcoPrescaleGetFrequency();
#endif
#if SRSS_BACKUP_S40E_LPECO_PRESENT
...
static cy_rslt_t _cyhal_clock_set_enabled_eco(cyhal_clock_t *clock, bool enabled, bool wait_for_lock)
{
    ...
    if (enabled)
    {
        if (0u != (SRSS_CLK_ECO_CONFIG & SRSS_CLK_ECO_CONFIG_ECO_EN_Msk))
        {
            ...
            if (wait_for_lock)
            {
                for (uint8_t t = 0; t < 3 && Cy_SysClk_EcoGetStatus() != CY_SYSCLK_ECOSTAT_STABLE; ++t)
                {
...
static bool _cyhal_clock_is_enabled_pll(const cyhal_clock_t *clock)
{
    ...
    #elif defined(COMPONENT_CAT1D)
    /* CAT1D device does not have FLL, so no index increment needed */
    if (clock->block == CYHAL_CLOCK_BLOCK_DPLL500)
    {
        return Cy_SysClk_PllIsEnabled(clock->channel + SRSS_NUM_DPLL_LP);
    }
    else if (clock->block == CYHAL_CLOCK_BLOCK_DPLL250)
    {
...
static cy_rslt_t _cyhal_clock_set_enabled_pll(cyhal_clock_t *clock, bool enabled, bool wait_for_lock)
{
    ...
    //pll_idx is the path mux index as used by PDL APIs, 0-indexed for CY_IP_MXS22SRSS
    uint32_t pll_idx = clock->channel;
    #else
    ...
    cy_stc_pll_manual_config_t cfg = {.lpPllCfg = &DpllLPConfig, .hpPllCfg = &DpllHPConfig};
    #else
    ...
    {
        uint32_t new_freq, old_freq;
        uint32_t div = (uint32_t)Cy_SysClk_ClkHfGetDivider(_CYHAL_CLOCK_CPU_HFCLK_IDX);
...
static uint32_t _cyhal_clock_get_frequency_pll(const cyhal_clock_t *clock)
{
    ...
    if (clock->block == CYHAL_CLOCK_BLOCK_DPLL500)
    {
        return Cy_SysClk_ClkPathGetFrequency(clock->channel + SRSS_NUM_DPLL_LP);
    }
    else if (clock->block == CYHAL_CLOCK_BLOCK_DPLL250)
    {
        return Cy_SysClk_ClkPathGetFrequency(clock->channel);
    }
    #endif
...
static cy_rslt_t _cyhal_clock_set_divider_mf(cyhal_clock_t *clock, uint32_t divider)
{
    ...
    if (divider <= _CYHAL_CLOCK_MAX_INT_DIV)
    {
...
static cy_rslt_t _cyhal_clock_set_divider_hf(cyhal_clock_t *clock, uint32_t divider)
{
    ...
    uint32_t old_div = (uint32_t)Cy_SysClk_ClkHfGetDivider(CY_SYSCLK_CLK_CORE_HF_PATH_NUM);
    uint32_t src = (uint32_t)Cy_SysClk_ClkHfGetSource(CY_SYSCLK_CLK_CORE_HF_PATH_NUM);
    ...
    if (CY_SYSCLK_CLK_CORE_HF_PATH_NUM == clock->channel)
        _cyhal_clock_update_system_state(true, old_freq, new_freq);
...
static cy_rslt_t _cyhal_clock_set_source_hf(cyhal_clock_t *clock, const cyhal_clock_t *source)
{
...
#endif
    {
        ...
        return CYHAL_CLOCK_RSLT_ERR_SOURCE;
    /* Only used if updating HFClk 0 */
    uint32_t div_hf0 = (uint32_t)Cy_SysClk_ClkHfGetDivider(CY_SYSCLK_CLK_CORE_HF_PATH_NUM);
    uint32_t old_src_hf0 = (uint32_t)Cy_SysClk_ClkHfGetSource(CY_SYSCLK_CLK_CORE_HF_PATH_NUM);
...
static cy_rslt_t _cyhal_clock_set_frequency_fast(cyhal_clock_t *clock, uint32_t hz, const cyhal_clock_tolerance_t *tolerance)
{
    ...
    if (CY_RSLT_SUCCESS == rslt)
    {
        ...
        #else
        uint32_t div_int = (div >> _CYHAL_CLOCK_FRAC_DIV_DIGITS) - 1;//Shift to find integer divider, subtract 1 as PDL is 0 indexed
...
static bool _cyhal_clock_is_enabled_eco_prescaler(const cyhal_clock_t *clock)
{
    CY_UNUSED_PARAMETER(clock);
    return Cy_SysClk_EcoPrescaleIsEnabled();
}
static cy_rslt_t _cyhal_clock_set_divider_eco_prescaler(cyhal_clock_t *clock, uint32_t divider)
{
    CY_UNUSED_PARAMETER(clock);
    if (divider <= _CYHAL_CLOCK_ECO_MAX_INT_DIV) //ECO Prescaler has a max divide of 1024 instead of 256
    {
        uint32_t divVal = divider - 1;
        Cy_SysClk_EcoPrescaleConfigure((uint32_t)true,divVal,0);
        SystemCoreClockUpdate();
        return CY_RSLT_SUCCESS;
    }
    return CYHAL_CLOCK_RSLT_ERR_FREQ;
}
static cy_rslt_t _cyhal_clock_set_enabled_eco_prescaler(cyhal_clock_t *clock, bool enabled, bool wait_for_lock)
{
    CY_UNUSED_PARAMETER(clock);
    CY_UNUSED_PARAMETER(wait_for_lock);
    cy_rslt_t rslt = CY_RSLT_SUCCESS;
    if(!_cyhal_clock_is_enabled_eco(clock) && enabled)
    {
        rslt = _cyhal_clock_set_enabled_eco(clock,enabled,wait_for_lock);
    }
    if(rslt == CY_RSLT_SUCCESS){
        if (enabled)
            {Cy_SysClk_EcoPrescaleConfigure(true,0,0);}
        else
            {Cy_SysClk_EcoPrescaleConfigure(false,0,0);}
    }
    return rslt;
}
static cy_rslt_t _cyhal_clock_set_frequency_eco_prescaler(cyhal_clock_t *clock, uint32_t hz, const cyhal_clock_tolerance_t *tolerance)
{
    CY_UNUSED_PARAMETER(clock);

    uint32_t div;
    cy_rslt_t rslt;
    uint32_t input_hz = Cy_SysClk_EcoPrescaleGetFrequency();
    rslt = _cyhal_clock_compute_div(((uint64_t)input_hz) << 5, hz, 13, tolerance, &div);
    ...
    if (CY_RSLT_SUCCESS == rslt)
    {
        uint32_t div_int = (div >> _CYHAL_CLOCK_FRAC_DIV_DIGITS) - 1;//Shift to find integer divider, subtract 1 as PDL is 0 indexed
        uint32_t div_frac = div & 0x1F;
        Cy_SysClk_EcoPrescaleConfigure(true, (uint8_t)div_int, (uint8_t)div_frac);
        Cy_SysLib_SetWaitStates(false, 340);
        SystemCoreClockUpdate();
    }
    return rslt;
...
{
    CY_UNUSED_PARAMETER(clock);
    static const cyhal_resource_inst_t *_CYHAL_CLOCK_SOURCE_ALT_ECO_PRESCALER[] = {&CYHAL_CLOCK_RSC_ECO,};
    *sources = _CYHAL_CLOCK_SOURCE_ALT_ECO_PRESCALER;
    *count = sizeof(_CYHAL_CLOCK_SOURCE_ALT_ECO_PRESCALER) / sizeof(_CYHAL_CLOCK_SOURCE_ALT_ECO_PRESCALER[0]);
    return CY_RSLT_SUCCESS;
}
#endif
...
{
    CY_UNUSED_PARAMETER(clock);
    return CY_SYSCLK_TIMER_CLK_FREQ;
}
...
static cy_rslt_t _cyhal_clock_get_sources_alt_sys_tick(const cyhal_clock_t *clock, const cyhal_resource_inst_t **sources[], uint32_t *count)
{
    ...
    static const cyhal_resource_inst_t *_CYHAL_CLOCK_SOURCE_ALT_SYS_TICK[] =
    {
        ...
        &CYHAL_CLOCK_RSC_HF[1], /* CPU clock */
#else
...
{
    .features = (cyhal_clock_feature_t)(CYHAL_CLOCK_FEATURE_ENABLE | CYHAL_CLOCK_FEATURE_DIVIDER | CYHAL_CLOCK_FEATURE_FREQUENCY),
    .is_enabled = _cyhal_clock_is_enabled_eco_prescaler,
    .set_enabled = _cyhal_clock_set_enabled_eco_prescaler,
    .get_frequency = _cyhal_clock_get_frequency_eco_prescaler,
    .set_frequency = _cyhal_clock_set_frequency_eco_prescaler,
