...
static void _cyhal_ezi2c_set_ezmode(cyhal_ezi2c_t *obj, bool enable)
{
    if (enable)
    {
        /*In EZ mode, a meta protocol is applied to the serial interface protocol. Interrupts are not triggered.
         * This meta protocol adds meaning to the data frames transferred by the serial interface protocol.
         * This mode is only applicable to slave functionality. 
         * In EZ mode, the slave can read from and write to an addressable memory structure of up to 256 bytes.*/
        SCB_CTRL(obj->base) |= SCB_CTRL_EZ_MODE_Msk;
    }
    else
    {
        /*In No-EZ mode, Interrupts are triggered.*/
         SCB_CTRL(obj->base) &= ~SCB_CTRL_EZ_MODE_Msk;
    }
}
...
static cy_rslt_t _cyhal_ezi2c_setup_resources(cyhal_ezi2c_t *obj, cyhal_gpio_t sda, cyhal_gpio_t scl, const cyhal_clock_t *clk)
{
    ...
    _cyhal_utils_enable_ip_block(&rsc_to_reserve);
...
#if defined (COMPONENT_CAT5)
        // This is a temporary workaround to enable interrupt based transfer. It should be removed when the hardware defaults to the expected mode.
        /* Configure EZI2C to be Interrupt based */
        _cyhal_ezi2c_set_ezmode(obj, false);
