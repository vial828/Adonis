...
* \{
* On CAT1D the OTP region is accessible in Secure mode.
...
#define _CYHAL_NVM_SECURE_REGION_MASK (0x10000000u)
#endif
...
{
#if defined(COMPONENT_CAT1D) && defined(_CYHAL_IMAGE_TYPE_SECURE)
return ((address | _CYHAL_NVM_SECURE_REGION_MASK ) >= region_info->start_address && (address | _CYHAL_NVM_SECURE_REGION_MASK ) < (region_info->start_address + region_info->size));
#else
...
cy_rslt_t cyhal_nvm_read(cyhal_nvm_t *obj, uint32_t address, uint8_t *data, size_t size)
{
    ...
    else
    {
        ...
        {
        #if (_CYHAL_DRIVER_AVAILABLE_NVM_RRAM)
            status = (cy_rslt_t)Cy_RRAM_TSReadByteArray(RRAMC0, address, data, size);
        #endif /* #if (_CYHAL_DRIVER_AVAILABLE_NVM_RRAM) */
        }
