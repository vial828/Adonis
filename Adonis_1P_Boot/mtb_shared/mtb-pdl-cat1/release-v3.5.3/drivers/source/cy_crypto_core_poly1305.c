...

{
    ...
    uint8_t *stateRemap;
    uint8_t *stateStateRemap;

    if((NULL != state))
    {
        stateRemap = (uint8_t *)CY_REMAP_ADDRESS_FOR_CRYPTO(state);
        stateStateRemap = (uint8_t *)CY_REMAP_ADDRESS_FOR_CRYPTO(state->state);

        Cy_Crypto_Core_V2_MemSet(base, (void*)stateStateRemap, 0U, (uint16_t)sizeof(cy_stc_crypto_poly_1305_buffers_t));
        Cy_Crypto_Core_V2_MemSet(base, (void*)stateRemap, 0U, (uint16_t)sizeof(cy_stc_crypto_poly_1305_state_t));
...
#if (((CY_CPU_CORTEX_M7) && defined (ENABLE_CM7_DATA_CACHE)) || CY_CPU_CORTEX_M55)
    CY_ALIGN(__SCB_DCACHE_LINE_SIZE) static cy_stc_crypto_poly_1305_state_t  state;
    CY_ALIGN(__SCB_DCACHE_LINE_SIZE) static cy_stc_crypto_poly_1305_buffers_t  buffer;

#else
    cy_stc_crypto_poly_1305_state_t state = {0};
    cy_stc_crypto_poly_1305_buffers_t buffer = {0};
#endif

#if (((CY_CPU_CORTEX_M7) && defined (ENABLE_CM7_DATA_CACHE)) || CY_CPU_CORTEX_M55)
        /* Flush the cache */
        SCB_CleanDCache_by_Addr((volatile void *)&state,(int32_t)sizeof(cy_stc_crypto_poly_1305_state_t));
        SCB_CleanDCache_by_Addr((volatile void *) &buffer,(int32_t)sizeof(cy_stc_crypto_poly_1305_buffers_t));
#endif
