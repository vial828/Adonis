...
#define AUTANALOG_DAC_BUF_POW(value)                                 ((value) <= CY_AUTANALOG_DAC_BUF_PWR_ULTRA_HIGH_RAIL)
#define AUTANALOG_DAC_VREF_MUX(value)                                ((value) <= CY_AUTANALOG_DAC_VREF_MUX_PRB_VREF1)
...
cy_en_autanalog_status_t Cy_AutAnalog_DAC_LoadStaticConfig(uint8_t dacIdx, const cy_stc_autanalog_dac_sta_t * dacStaCfg)
{
    ...
    if (NULL != dacStaCfg)
    {
        ...
        if (CY_AUTANALOG_SUCCESS == result)
        {
            ...
            CY_ASSERT_L3(AUTANALOG_DAC_BUF_POW(dacStaCfg->refBuffPwr));
            CY_ASSERT_L3(AUTANALOG_DAC_BUF_POW(dacStaCfg->outBuffPwr));
            ...
            if ((CY_AUTANALOG_DAC_VREF_VDDA != dacStaCfg->vrefSel) &&
                ((CY_AUTANALOG_DAC_BUF_PWR_OFF != dacStaCfg->refBuffPwr) ||
                 (CY_AUTANALOG_DAC_BUF_PWR_OFF != dacStaCfg->outBuffPwr)))
            {
...
cy_en_autanalog_status_t Cy_AutAnalog_DAC_Data_GetDriveModeArray(uint8_t dacIdx, uint16_t dataIdx, uint16_t numSamples, cy_en_autanalog_dac_out_drive_mode_t * driveModeArray)
{
    ...
    if ((dataIdx + numSamples <= CY_AUTANALOG_DAC_DATA_SAMPLES_NUM) && (NULL != driveModeArray))
    {
        ...
        if (CY_AUTANALOG_SUCCESS == result)
        {
            for (sampleIdx = 0U; sampleIdx < numSamples; sampleIdx++)
            {
                CY_MISRA_DEVIATE_LINE('MISRA C-2012 Rule 10.3','Review shows that type conversion from uint32_t to enum does not have any negative drawbacks');
                driveModeArray[sampleIdx] = _FLD2VAL(DAC_SRAM_OUT_DM, AUTANALOG_DAC_LUT(baseAddr, dataIdx++));
            }
        }
    }
