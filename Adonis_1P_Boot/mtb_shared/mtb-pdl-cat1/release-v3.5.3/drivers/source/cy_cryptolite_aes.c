...

{
    ...
    *dstLength = outputLength;
    ...
    {
        Cy_Cryptolite_Vu_memcpy((void*)&aesState->buffers->unProcessedData[aesState->unProcessedBytes], (void*)src, (uint16_t)srcSize);
    ...
    else
    {
        if(aesState->unProcessedBytes > 0u)
        {
            ...
            Cy_Cryptolite_Vu_memcpy((void*)&aesState->buffers->unProcessedData[aesState->unProcessedBytes], (void*)src, (uint16_t)bytes_to_copy);
        ...
        {
            status = Cy_Cryptolite_Aes_Ecb(base, dst, src, aesState);
        ...
        if(srcSize > 0u)
        {
            Cy_Cryptolite_Vu_memcpy((void*)aesState->buffers->unProcessedData, (void*)src, (uint16_t)srcSize);
            aesState->unProcessedBytes = srcSize;
        }
    }
                                            ...
                                            cy_stc_cryptolite_aes_state_t *aesState)
{
    ...
    Cy_Cryptolite_Vu_memcpy((void*)&aesState->buffers->iv, (void*)iv, (uint16_t)CY_CRYPTOLITE_AES_BLOCK_SIZE);
    ...
    *dstLength = outputLength;
    ...
    else
    {
        ...
        if(cnt != 0u)
        {
            ...
            src += cnt * CY_CRYPTOLITE_AES_BLOCK_SIZE;
        ...
        if(srcSize > 0u)
        {
            Cy_Cryptolite_Vu_memcpy(aesState->buffers->unProcessedData, (void*)src, (uint16_t)srcSize);
            aesState->unProcessedBytes = srcSize;
        }
    }
...

{
    ...
    else
    {
        if(aesState->unProcessedBytes > 0u)
        {
            ...
            status = Cy_Cryptolite_Aes_Cbc(base, CY_CRYPTOLITE_AES_BLOCK_SIZE, aesState->buffers->iv,
                                            aesState->buffers->temp, aesState->buffers->unProcessedData, aesState);
            if(CY_CRYPTOLITE_SUCCESS != status)
            {
                return status;
            }
        ...
        {
            status = Cy_Cryptolite_Aes_Cbc(base, CY_CRYPTOLITE_AES_BLOCK_SIZE, aesState->buffers->iv,
                                            aesState->buffers->temp, src, aesState);
            if(CY_CRYPTOLITE_SUCCESS != status)
            {
                return status;
            }
        ...
        if(srcSize > 0u)
        {
            Cy_Cryptolite_Vu_memcpy(aesState->buffers->unProcessedData, (void*)src, srcSize%CY_CRYPTOLITE_AES_BLOCK_SIZE);
            aesState->unProcessedBytes = srcSize;
        }
    }
                                             ...
                                             cy_stc_cryptolite_aes_state_t *aesState)
{
    ...
    if(aesState->unProcessedBytes !=0u)
    {
        ...
        {
            dst[i] = src[i] ^ aesState->buffers->unProcessedData[aesState->unProcessedBytes + i];
        }
        ...
        {
            Cy_Cryptolite_Vu_memcpy(&aesState->buffers->iv[aesState->unProcessedBytes], src, (uint16_t)bytes_rem);   
        }
    ...
    if(cnt != 0u)
    {
        tmpResult = Cy_Cryptolite_Aes_Cfb(base,aesState->dirMode, cnt * CY_CRYPTOLITE_AES_BLOCK_SIZE,
                                             aesState->buffers->iv, dst, src, aesState);
    ...
    if(srcSize > 0u)
    {
        ...
        {
            dst[i] = src[i] ^ aesState->buffers->unProcessedData[i];
        }
        ...
        {
            Cy_Cryptolite_Vu_memcpy(&aesState->buffers->iv, src, (uint16_t)srcSize);
        }
...

{
    ...
    while(srcSize>0u)
    {
        ...
        if(aesState->unProcessedBytes == 0u && cnt>0u)
        {
            status = Cy_Cryptolite_Aes_Ctr(CRYPTOLITE, cnt*CY_CRYPTOLITE_AES_BLOCK_SIZE, &srcOffset, aesState->buffers->iv, dst, src, aesState);
            if(CY_CRYPTOLITE_SUCCESS != status)
            {
                return status;
            }
        ...
        *dst++ = *src++ ^ aesState->buffers->unProcessedData[aesState->unProcessedBytes];
