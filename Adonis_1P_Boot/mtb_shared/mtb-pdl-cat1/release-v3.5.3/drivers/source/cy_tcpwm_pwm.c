    ...
    if ((NULL != base) && (NULL != config))
    {
            ...
            if (CY_TCPWM_PWM_MODE_DEADTIME == config->pwmMode)
            {
                ...
                TCPWM_GRP_CNT_DT_BUFF(base, grp, cntNum) =
                        ...
                        _VAL2FLD(TCPWM_GRP_CNT_V2_DT_DT_LINE_COMPL_OUT, config->deadTimeClocksBuff));
#endif /* defined (CY_IP_MXS40TCPWM) */
            }
#if !defined (CY_IP_MXS40TCPWM)
            else
...
/*******************************************************************************
* Function Name: Cy_TCPWM_HR_Set_Enable
****************************************************************************//**
*
* configures high resolution PWM. applicable only when GRP_HRPWM_PRESENT and in PWM and PWM_DT mode
*
* \param base
* The pointer to a TCPWM instance.
*
...
{
    uint32_t grp = TCPWM_GRP_CNT_GET_GRP(cntNum);
    cy_en_tcpwm_status_t status = CY_TCPWM_UNSUPPORTED_FEATURE;
    bool hrpwm_present = ((bool)TCPWM_GRP_HRPWM_PRESENT(grp));
    if(hrpwm_present)
    {
        if (enable)
        {
            TCPWM_GRP_CNT_HRPWM_CTRL(base, grp, cntNum) |= TCPWM_GRP_CNT_HRPWM_CTRL_HRPWM_EN_Msk;
        }
        else
        {
            TCPWM_GRP_CNT_HRPWM_CTRL(base, grp, cntNum) &= ~TCPWM_GRP_CNT_HRPWM_CTRL_HRPWM_EN_Msk;
...
/*******************************************************************************
* Function Name: Cy_TCPWM_HR_Set_Input_Freq
****************************************************************************//**
*
* HRPWM Operation Frequency
* Selects the frequency of operation for HRPWM. Micro tick is adjusted based on this information.
* Input Frequency is the frequency of CLK_OUT from TCPWM counter. CLK_OUT = CLK_PERI = CLK_HF
*
* \param base
* The pointer to a TCPWM instance.
*
...
*
* \param hrpwm_freq
* HRPWM operating frequency. \ref cy_en_hrpwm_operating_frequency_t
*
* \return
* Error / Status code. See cy_en_tcpwm_status_t.
*******************************************************************************/
cy_en_tcpwm_status_t Cy_TCPWM_HR_Set_Input_Freq(TCPWM_Type *base, uint32_t cntNum, cy_en_hrpwm_operating_frequency_t hrpwm_freq)
{
    ...
    if(hrpwm_present)
    {
        TCPWM_GRP_CNT_HRPWM_CTRL(base, grp, cntNum) |= _VAL2FLD(TCPWM_GRP_CNT_HRPWM_CTRL_FREQ_SEL, hrpwm_freq);
        status = CY_TCPWM_SUCCESS;
    }
    return status;
}
