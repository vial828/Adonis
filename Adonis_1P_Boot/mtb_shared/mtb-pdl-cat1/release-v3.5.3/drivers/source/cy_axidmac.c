...
cy_en_axidmac_status_t Cy_AXIDMAC_Descriptor_Init(cy_stc_axidmac_descriptor_t * descriptor, const cy_stc_axidmac_descriptor_config_t * config)
{
    ...
    if ((NULL != descriptor) && (NULL != config))
    {
        ...
        descriptor->ctl =
            _VAL2FLD(SAXI_DMAC_CH_DESCR_CTL_WAIT_FOR_DEACT, config->retrigger) |
            _VAL2FLD(SAXI_DMAC_CH_DESCR_CTL_INTR_TYPE, config->interruptType) |
            _VAL2FLD(SAXI_DMAC_CH_DESCR_CTL_TR_OUT_TYPE, config->triggerOutType) |
            _VAL2FLD(SAXI_DMAC_CH_DESCR_CTL_TR_IN_TYPE, config->triggerInType) |
           _BOOL2FLD(SAXI_DMAC_CH_DESCR_CTL_DATA_PREFETCH, config->dataPrefetch) |
            _VAL2FLD(SAXI_DMAC_CH_DESCR_CTL_CH_DISABLE, config->channelState) |
            _VAL2FLD(SAXI_DMAC_CH_DESCR_CTL_DESCR_TYPE, config->descriptorType);
           ...
           (CY_AXIDMAC_3D_MEMORY_COPY == config->descriptorType))
        {
            ...
            descriptor->xIncr = _VAL2FLD(SAXI_DMAC_CH_DESCR_X_INCR_SRC_X, config->srcXincrement) |
                                _VAL2FLD(SAXI_DMAC_CH_DESCR_X_INCR_DST_X, config->dstXincrement);
        }
        ...
        if(CY_AXIDMAC_3D_MEMORY_COPY == config->descriptorType)
        {
            ...
            descriptor->yIncr = _VAL2FLD(SAXI_DMAC_CH_DESCR_Y_INCR_SRC_Y, config->srcYincrement) |
                                _VAL2FLD(SAXI_DMAC_CH_DESCR_Y_INCR_DST_Y, config->dstYincrement);
        }
...
cy_en_axidmac_status_t Cy_AXIDMAC_Channel_Init(AXI_DMAC_Type * base, uint32_t channel, cy_stc_axidmac_channel_config_t const * config)
{
    ...
    if ((NULL != base) && (CY_AXIDMAC_IS_CH_NR_VALID(channel)) && (NULL != config) && (NULL != config->descriptor))
    {
        ...
        AXIDMAC_CH_CTL(base, channel) = _VAL2FLD(SAXI_DMAC_CH_CTL_PRIO,    config->priority) |
                                    _BOOL2FLD(SAXI_DMAC_CH_CTL_ENABLED, config->enable)   |
                                    _BOOL2FLD(SAXI_DMAC_CH_CTL_B,       config->bufferable);
        ret = CY_AXIDMAC_SUCCESS;
    }
...
uint32_t Cy_AXIDMAC_Descriptor_GetXloopDataCount(cy_stc_axidmac_descriptor_t const * descriptor)
{
    ...
    CY_ASSERT_L1(CY_AXIDMAC_1D_MEMORY_COPY != Cy_AXIDMAC_Descriptor_GetDescriptorType(descriptor));
     ...
     retVal = _FLD2VAL(SAXI_DMAC_CH_DESCR_X_SIZE_X_COUNT, descriptor->xSize) + 1UL;
...
void Cy_AXIDMAC_Descriptor_SetXloopDataCount(cy_stc_axidmac_descriptor_t * descriptor, uint32_t xCount)
{
    ...
    descriptor->xSize = _VAL2FLD(SAXI_DMAC_CH_DESCR_X_SIZE_X_COUNT, xCount - 1UL);
}
...
void Cy_AXIDMAC_Descriptor_SetNextDescriptor(cy_stc_axidmac_descriptor_t * descriptor, cy_stc_axidmac_descriptor_t const * nextDescriptor)
{
    ...
    switch((cy_en_axidmac_descriptor_type_t)_FLD2VAL(SAXI_DMAC_CH_DESCR_CTL_DESCR_TYPE, descriptor->ctl))
    {
...
void Cy_AXIDMAC_Descriptor_SetDescriptorType(cy_stc_axidmac_descriptor_t * descriptor, cy_en_axidmac_descriptor_type_t descriptorType)
{
    ...
    if (descriptorType != Cy_AXIDMAC_Descriptor_GetDescriptorType(descriptor)) /* Do not perform if the type is not changed */
    {
        ...
        CY_REG32_CLR_SET(descriptor->ctl, SAXI_DMAC_CH_DESCR_CTL_DESCR_TYPE, descriptorType);
