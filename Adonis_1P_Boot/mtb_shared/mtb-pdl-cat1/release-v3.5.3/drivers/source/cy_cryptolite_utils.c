                        ...
                        uint32_t size)
{
    ...
    if((((uint32_t)dst & 0x3UL) == 0UL) && (((uint32_t)src & 0x3UL) == 0UL))
    {
        status = Cy_Cryptolite_Vu_mov_hw(base, &p_struct, dst, CY_CRYPTOLITE_WORD_SIZE_OF_BYTES(size) , src, CY_CRYPTOLITE_WORD_SIZE_OF_BYTES(size));
    }
    else
    #endif
    {
