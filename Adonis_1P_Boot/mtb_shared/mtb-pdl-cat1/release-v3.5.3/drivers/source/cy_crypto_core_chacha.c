...

{
    ...
    CY_ALIGN(__SCB_DCACHE_LINE_SIZE) static const uint32_t p_constants[4] = {0x61707865, 0x3320646e, 0x79622d32, 0x6b206574 };
#else
                                     ...
                                     uint32_t inputSize, uint8_t *output)
{
    cy_en_crypto_status_t tmpResult = CY_CRYPTO_BAD_PARAMS;
#if (((CY_CPU_CORTEX_M7) && defined (ENABLE_CM7_DATA_CACHE)) || CY_CPU_CORTEX_M55)
    CY_ALIGN(__SCB_DCACHE_LINE_SIZE) static cy_stc_crypto_chacha_state_t state;
    CY_ALIGN(__SCB_DCACHE_LINE_SIZE) static cy_stc_crypto_v2_chacha_buffers_t buffer;
#else
    cy_stc_crypto_chacha_state_t state;
    cy_stc_crypto_v2_chacha_buffers_t buffer;
#endif
