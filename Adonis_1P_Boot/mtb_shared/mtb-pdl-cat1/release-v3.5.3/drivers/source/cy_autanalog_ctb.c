...
#define AUTANALOG_CTB_OA_INV_INP_DIR(value)                   ((value) <= CY_AUTANALOG_CTB_OA_INV_PIN_BJT)
...
#define AUTANALOG_CTB_OA_CTBBUS_MUX_OUT(value)                ((value) <= CY_AUTANALOG_CTB_OA_MUX_OUT_BJT)
...
cy_en_autanalog_status_t Cy_AutAnalog_CTB_LoadStaticConfig(uint8_t ctbIdx, const cy_stc_autanalog_ctb_sta_t * ctbStaCfg)
{
    ...
    if (NULL != ctbStaCfg)
    {
        ...
        {
            /* Enables the required start-up/settling delay for the enabled OPAMP */
            if (0U == ctbIdx)
            {
