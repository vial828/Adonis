                                            ...
                                            uint8_t const *key, cy_stc_cryptolite_aes_ccm_state_t *aesCcmState)
{
    ...
    status = Cy_Cryptolite_Aes_Init(base, key, &aesCcmState->aesCbcMacState, aesCcmState->aesCbcMacState.buffers);
    ...
    status = Cy_Cryptolite_Aes_Init(base, key, &aesCcmState->aesCtrState, aesCcmState->aesCtrState.buffers);
    if(CY_CRYPTOLITE_SUCCESS != status)
    {
        return status;
    }
                                            ...
                                            cy_stc_cryptolite_aes_ccm_state_t *aesCcmState)
{
    ...
    Cy_Cryptolite_Vu_memcpy((void*)&aesCcmState->ctr[1], (void*)iv, (uint16_t)ivSize);

    Cy_Cryptolite_Vu_memset (aesCcmState->y, 0u, CY_CRYPTOLITE_AES_BLOCK_SIZE);
    Cy_Cryptolite_Vu_memcpy((void*)&aesCcmState->y[1], (void*)iv, (uint16_t)ivSize);
...

{
    ...
    status = Cy_Cryptolite_Aes_CbcMac_Update(base, aadSize, aad, &aesCcmState->aesCbcMacState);
    if(CY_CRYPTOLITE_SUCCESS != status)
    {
        return status;
    }
...

{
    ...
    status =  Cy_Cryptolite_Aes_Ctr_Update(base, srcSize, dst, src, &aesCcmState->aesCtrState);
    if(CY_CRYPTOLITE_SUCCESS != status)
    {
        return status;
    }
...

{
    ...
    status = Cy_Cryptolite_Aes_Ccm_Start(base, dirMode, ivSize, iv, aesCcmState);    
    ...
    status = Cy_Cryptolite_Aes_Ccm_Update_Aad(base, aadSize, aad, aesCcmState);
    ...
    status = Cy_Cryptolite_Aes_Ccm_Update(base, srcSize, dst, src, aesCcmState);
