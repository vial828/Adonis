...
'The case for CAT1D is false because the size of work, \
 flash subsection is zero but will be true if devices have valid size.')
...
#ifdef WA__PM_0159902_6
    (void)Cy_IPC_Drv_LockRelease (Cy_IPC_Drv_GetIpcBaseAddress(CY_IPC_CHAN_RRAM_LOCK), (uint32_t)(1UL << CY_IPC_INTR_RRAM_LOCK));
#endif
}
...
****************************************************************************//**
*
* Returns status on RRAM internal read operation ECC-status.
* ECC single, double, triple bit failures in read operation
* are treated as only warnings.
*
*******************************************************************************/
__STATIC_INLINE cy_en_rram_ecc_status_t CY_RRAM_ReadOperationECCStatus(RRAMC_Type * base)
{
    return (cy_en_rram_ecc_status_t)(RRAM_NVM_STATUS(base) &
            RRAMC_RRAM_SFR_NVM_STATUS_ECC4MRD_Msk);
}
...
static cy_stc_rram_rgn_info_t Cy_RRAM_GetNVMSubRegionInfo(uint32_t addr)
{
    ...
    if((addr >= CY_RRAM_MAIN_HOST_NS_START_ADDRESS) && (addr <= CY_RRAM_MAIN_HOST_NS_END_ADDRESS))
    {
...
static cy_en_rram_status_t Cy_RRAM_ReadBlock(cy_stc_rram_config_t * config, uint8_t * data, uint32_t pos, uint32_t numBytes)
{
    ...
    if(config->tearingSafe)
    {
        ...
        RRAM_NVM_ADDR(config->base) = Cy_RRAM_GetPhyBase(config->subRegionBase) +
                                     (((config->addr & ~0x0FUL) - config->subRegionBase) / CY_RRAM_BLOCK_SIZE_BYTES);

        /* Set operation mode.to indirect write. */
        Cy_RRAM_SetOperationMode(config->base, CY_RRAM_INDIRECT_WRITE);

        /* Check for busy status. */
        while((Cy_RRAM_GetStatus(config->base) & 1UL) == RRAMC_RRAM_SFR_NVM_STATUS_BUSY_Msk) {}

        /* Validate ECC status.*/
        if((CY_RRAM_ReadOperationECCStatus(config->base) != CY_RRAM_ECC_READ_BLOCK_MULTIPLE_BIT_FAIL) &&
            (CY_RRAM_ReadOperationECCStatus(config->base) != CY_RRAM_ECC_READ_BLOCK_TRIPLE_BIT_FAIL))
        {
            /* Update status if there is no uncorrectable ECC error. */
...
static cy_en_rram_status_t Cy_RRAM_SubRegionReadData(cy_stc_rram_config_t * config, uint8_t * data, uint32_t numBytes)
{
    ...
    else
    {
        /* The status is being set to SUCCESS in order to make one of the
           entry check for only read 16-byte aligned blocks case as true.
...
/* Default ECC Check Enabled. Application can overwrite this to avoid depending on DLT app */
...
{
    cy_en_rram_status_t status = CY_RRAM_ECC_FAIL;
    uint32_t timeout = CY_RRAM_OPERATION_TIMEOUT;
    cy_rram_blk_buf_t blk;
    ...
    {
        /* Check for the actual write completion. */
        ...
        /* In case of Double or Triple Bit error, retry indirect write operation */
        if(verrStatus == CY_RRAM_DOUBLE_BIT_ERROR ||
           verrStatus == CY_RRAM_TRIPLE_BIT_ERROR)
        {
            nvm_addr = (uint32_t *)config->addr;
            timeout = CY_RRAM_OPERATION_TIMEOUT;
            ...
            verrStatus = CY_RRAM_GetFailBitInfo(config->base);
        }

        if(verrStatus == CY_RRAM_ECC_UNCORRECTABLE_BIT_ERROR ||
           verrStatus == CY_RRAM_ECC_ADDR_MISMATCH_ERROR ||
...
cy_en_rram_status_t Cy_RRAM_Init(RRAMC_Type * base, cy_en_rram_vmode_t vmode, bool retrvAlgoInfo)
{
    ...
    /* Wait for the retrieve operation to finish. No timeout in SW required.
    * The HW FSM always terminates after predefined number of steps
    * and clears BUSY flag.
    */
    while(Cy_RRAM_IsBusy(base))
    {
    }
