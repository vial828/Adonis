                                              ...
                                              uint8_t *processedMessage)
{
    ...
    {
        Cy_Cryptolite_Rsa_Coeff(base, cfContext, key->barretCoefPtr, key->inverseModuloPtr, key->rBarPtr, key->moduloPtr, cfContext->p_buffer, key->moduloLength);
        key->preCalculatedCoeff = true;
    }

    Cy_Cryptolite_Rsa_Exp(base, cfContext, processedMessage, message, key->pubExpPtr, key->barretCoefPtr, key->inverseModuloPtr, key->rBarPtr, key->moduloPtr, cfContext->p_buffer, key->moduloLength, key->pubExpLength);
