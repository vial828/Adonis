...
#if defined (CY_IP_M4CPUSS) || defined (CY_IP_MXS40FLASHC)
...
#if defined (CY_IP_MXS40FLASHC)
/******** This section will be part of boot rom header file will be deleted later  *******/
/* boot rom header file section start */
typedef struct {
    uint32_t flags;    /* FLAGS to pass to flash functions, RWW vs STALL, BLOCKING vs NON-BLOCKING */

    /* For refresh feature */
    uint32_t min_count[1];         /* Write count of youngest page */
    uint32_t min_page[1];          /* Page with min_count.  Must refresh min_count if this page is written */
    uint32_t max_count[1];         /* Write count of oldest(current write) page */
    uint32_t scratch_index[1];     /* Wear leveling counter for scratch rows */
    uint32_t row_address;          /* An address of most recently erased page, or the page currently getting refreshed  */
    uint32_t endurance_counter;    /* An endurance counter of most recently erased page */

    /* For non-blocking Flash operations */
    uint32_t current_state;     /* REFRESH_COMPLETE, BUSY_ERASE_SPARE, BUSY_PROGRAM_SPARE, BUSY_ERASE_ROW, BUSY_PROGRAM_ROW */
    uint32_t status;            /* Last operation status */
    uint32_t callback_ipc_channel_idx; /*  value of the acquired IPC channel structure */
    uint32_t callback_ipc_intr_idx ;   /* value of the IPC interrupt structure which should trigger an IPC RELEASE interrupt  */
    /* ... */
} flash_context_t;
...
/* Initializes a global flash context structure, updates oldest/youngest page counters.  If there are multiple flash users at different PCs, they must share the same context structure or call flash_init before every write. */
static cyboot_flash_result_t cyboot_flash_init(flash_context_t *context) ;

/* If refresh is in progress, continues refresh.  If not, tests for refresh and starts if needed.  This is a non-blocking function. */ 
static cyboot_flash_result_t cyboot_flash_refresh(uint32_t sector, flash_context_t *context);

/* Erases the entire sector specified by address */
static cyboot_flash_result_t cyboot_erase_sector    (uint32_t address);

/* Erases the sub sector specified by address */
static cyboot_flash_result_t cyboot_erase_sub_sector    (uint32_t address);

/* boot rom header file section End */

#define HASH_CALC_DIVISOR 127U
#define HASH_CALC_DIVISOR_LEN 7U
#define IPC_INTERRUPT          cpuss_interrupts_ipc_dpslp_0_IRQn
#define IPC_CHANNEL    0U
#define IPC_INTERRPT_INDEX 0U
#define REG8(addr)                      ( *( (volatile uint8_t  *)(addr) ) )
static flash_context_t bootrom_flash_context;
static bool refresh_feature_enable;

/* This handler is used by boot rom to notify flash driver about the completion of flash operation */
static void Cy_IPC_Flash_NotifyHandler(void);
/* To calculate hash */
...
/* This handler is used by boot rom to notify flash driver about the completion of flash operation */
/*******************************************************************************
* Function Name: Cy_IPC_Flash_NotifyHandler
****************************************************************************//**
*
* Interrupt handler for non blocking operations.
* There are no system calls in cat1b devices.
* It will be a direct call to BOOT ROM functions.
* For non blocking operations boot rom notifies on ipc interrupt
* Status of the operation is updated in the boot rom context structure.
* This interrupt can be used to notify the application on the completion status.
* Currently adopting the existing design of application polling for the status by calling
* \ref Cy_Flash_IsOperationComplete
*
*******************************************************************************/
static void Cy_IPC_Flash_NotifyHandler(void)
{
...
cy_en_flashdrv_status_t Cy_Flash_Init(bool refresh_enable)
{
    ...
    cyboot_flash_result_t bootrom_status;
    /* Setup the  IPC Interrupt */
    const cy_stc_sysint_t ipcIntConfig =
    {
        .intrSrc = IPC_INTERRUPT,
        .intrPriority = 1U,
    };
    refresh_feature_enable = refresh_enable;
    bootrom_flash_context.callback_ipc_channel_idx = IPC_CHANNEL;
    bootrom_flash_context.callback_ipc_intr_idx    = IPC_INTERRPT_INDEX;
    sysStatus = Cy_SysInt_Init(&ipcIntConfig, Cy_IPC_Flash_NotifyHandler);
    if(CY_SYSINT_SUCCESS != sysStatus)
    {
        return CY_FLASH_DRV_ERR_UNC;
    }
    NVIC_EnableIRQ(IPC_INTERRUPT);
    bootrom_status = cyboot_flash_init(&bootrom_flash_context);
    if(bootrom_status != CYBOOT_FLASH_SUCCESS)
    {
        return CY_FLASH_DRV_ERR_UNC;
    }
...
cy_en_flashdrv_status_t Cy_Flash_EraseRow(uint32_t rowAddr)
{
    ...
    if (Cy_Flash_BoundsCheck(rowAddr) != false)
    {
        ...
        cyboot_flash_result_t status = ROM_FUNC->cyboot_flash_erase_row(rowAddr, &boot_rom_context);
...
cy_en_flashdrv_status_t Cy_Flash_StartEraseRow(uint32_t rowAddr)
{
    ...
    if (Cy_Flash_BoundsCheck(rowAddr) != false)
    {
        ...
        cyboot_flash_result_t status = ROM_FUNC->cyboot_flash_erase_row_start(rowAddr, &boot_rom_context);
        if(status == CYBOOT_FLASH_SUCCESS)
        {
            result = CY_FLASH_DRV_OPERATION_STARTED;
        }
...
#else
        /* works only with blocking mode and not supported with refresh feature enabled */
        boot_rom_context.flags = CYBOOT_FLAGS_BLOCKING;
        cyboot_flash_result_t status = cyboot_erase_sector(sectorAddr);
        if(status == CYBOOT_FLASH_SUCCESS)
        {
            result = CY_FLASH_DRV_SUCCESS;
        }
...
#else
        /* works only with blocking mode and not supported with refresh feature enabled */
        cyboot_flash_result_t status = cyboot_erase_sub_sector(subSectorAddr);
        if(status == CYBOOT_FLASH_SUCCESS)
        {
            result = CY_FLASH_DRV_SUCCESS;
        }
...
cy_en_flashdrv_status_t Cy_Flash_ProgramRow(uint32_t rowAddr, const uint32_t* data)
{
    ...
    if ((Cy_Flash_BoundsCheck(rowAddr) != false) && (NULL != data))
    {
        ...
        bootrom_flash_context.flags = CYBOOT_FLAGS_BLOCKING;
        boot_rom_context.flags = CYBOOT_FLAGS_BLOCKING;
        cyboot_flash_result_t status = ROM_FUNC->cyboot_flash_program_row(rowAddr, (void *)data, &boot_rom_context);
        if(status == CYBOOT_FLASH_SUCCESS)
        {
            result = CY_FLASH_DRV_SUCCESS;
        }
...
cy_en_flashdrv_status_t Cy_Flash_WriteRow(uint32_t rowAddr, const uint32_t* data)
{
    ...
    if ((Cy_Flash_BoundsCheck(rowAddr) != false) && (NULL != data))
    {
        ...
        bootrom_flash_context.flags = CYBOOT_FLAGS_BLOCKING;
        boot_rom_context.flags = CYBOOT_FLAGS_BLOCKING;
        cyboot_flash_result_t status = ROM_FUNC->cyboot_flash_write_row(rowAddr, (void *)data, &boot_rom_context);
        if(status == CYBOOT_FLASH_SUCCESS)
        {
            result = CY_FLASH_DRV_SUCCESS;
        }
...
#else
        boot_rom_context.flags = CYBOOT_FLAGS_NON_BLOCKING;
        cyboot_flash_result_t status = ROM_FUNC->cyboot_flash_program_row_start(rowAddr, (void *)data, &boot_rom_context);
        if(status == CYBOOT_FLASH_SUCCESS)
        {
            result = CY_FLASH_DRV_OPERATION_STARTED;
        }
...
#else
        uint32_t startAddr = (uint32_t)data;
        uint32_t endAddr = startAddr + numberOfBytes -1U;/* max 16MB size allowed to take care of overflow */
        if((Cy_Flash_BoundsCheck(endAddr))  && (Cy_Flash_BoundsCheck(startAddr)))
        {
...
#else    
    if(( CY_FLASH_MAIN_START_ADDRESS <= flashAddr) && (flashAddr < CY_FLASH_MAIN_END_ADDRESS))
    {
        return(1UL);
    }
    return (0UL);
...
#else
    result = Cy_Flash_ProcessOpcode(bootrom_flash_context.status);
#endif /* !defined (CY_IP_MXS40FLASHC) */
    return (result);
}
...
#else
    return (bootrom_flash_context.status);
#endif /* !defined (CY_IP_MXS40FLASHC) */
}
...
****************************************************************************//**
*
* Refreshes the flash rows which were not updated by the user code.
...
cy_en_flashdrv_status_t Cy_Flash_Refresh(uint32_t flashAddr)
{
    ...
    if(refresh_feature_enable)
    {
        if(( CY_FLASH_MAIN_START_ADDRESS <= flashAddr) && (flashAddr < CY_FLASH_MAIN_END_ADDRESS))
        {
            result = (cy_en_flashdrv_status_t)cyboot_flash_refresh(0U, &bootrom_flash_context);
        }
        else
        {
            result = CY_FLASH_DRV_REFRESH_NOT_SUPPORTED;
        }
    }
    return result;
}
...
/* Following functions are defined in boot rom code and have to be removed once bootrom code is available. */
...
* \return none
*******************************************************************************/
void Cy_Flashc_SetMain_Flash_Mapping(cy_en_bankmode_t mapping)
{
