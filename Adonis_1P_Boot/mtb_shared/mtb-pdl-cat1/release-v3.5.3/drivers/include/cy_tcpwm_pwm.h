...
typedef enum
{
    CY_TCPWM_OUTPUT_IS_LOW               = 0UL,   /**< Line Out and Line Compl Out is Low during the kill and counter is disabled. */
    CY_TCPWM_OUTPUT_LINEOUT_IS_HIGH      = 1UL,   /**< Line Out is High during the kill and counter is disabled. */
    CY_TCPWM_OUTPUT_LINECMPOUT_IS_HIGH   = 2UL,   /**< Line Compl Out is High during the kill and counter is disabled. */
    CY_TCPWM_OUTPUT_IS_HIGH              = 3UL,   /**< Line Out and Line Compl Out is Hig during the kill and counter is disabled. */
...
/* HRPWM Functions Only Available in PWM and PWM DT mode.*/
cy_en_tcpwm_status_t Cy_TCPWM_HR_Set_Enable(TCPWM_Type *base, uint32_t cntNum, bool enable);
cy_en_tcpwm_status_t Cy_TCPWM_HR_Set_Input_Freq(TCPWM_Type *base, uint32_t cntNum, cy_en_hrpwm_operating_frequency_t hrpwm_freq);
...
****************************************************************************//**
*
* Writes the dead time Buff value for PWM. This is the number of clock cycles between
* PWM_n (line_compl) going LOW and PWM (line) going HIGH.
* Will be swapped with Line out on a terminal count event with an actively pending switch event.
*
* \param base
* The pointer to a TCPWM instance.
*
...
__STATIC_INLINE void Cy_TCPWM_PWM_PWMDeadTimeBuff (TCPWM_Type const *base, uint32_t cntNum, uint32_t deadTime)
{
    ...
    TCPWM_GRP_CNT_DT_BUFF(base, grp, cntNum) =
                        (_VAL2FLD(TCPWM_GRP_CNT_V2_DT_DT_LINE_OUT_L, (uint8_t)(deadTime)) |
                        _VAL2FLD(TCPWM_GRP_CNT_V2_DT_DT_LINE_OUT_H, (uint8_t)(deadTime >> 8)) |
                        _VAL2FLD(TCPWM_GRP_CNT_V2_DT_DT_LINE_COMPL_OUT, deadTime));
