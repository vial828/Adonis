...
cy_rslt_t ota_smif_erase(uint32_t offset, uint32_t length)
{
	...
	/* commented by vincent.he, 20250211
	*/
    // sm_log(SM_LOG_DEBUG, "ota_smif_erase offset 0x%x, length 0x%x, and IS_FLAG_SET: %d, FLAG_HAL_INIT_DONE %d\r\n", offset, length, IS_FLAG_SET(FLAG_HAL_INIT_DONE), FLAG_HAL_INIT_DONE);
    ...
    if (IS_FLAG_SET(FLAG_HAL_INIT_DONE))
    {
        ...
        else
        {
            ...
            sm_log(SM_LOG_DEBUG, "Cy_SMIF_MemEraseSector offset 0x%x, length 0x%x\r\n", offset, length);//modify by vincent.he, 20250211
            cy_en_smif_result = Cy_SMIF_MemEraseSector(SMIF0,
                                                  smifBlockConfig.memConfig[MEM_SLOT],
                                                  offset, length, &ota_QSPI_context);
            //sm_log(SM_LOG_DEBUG, "Cy_SMIF_MemEraseSector cy_en_smif_result 0x%x\r\n", cy_en_smif_result);
        }
