...
//wanggankun@2024-8-21, note, flash layout with c code
                     ...
                     const void *src, uint32_t len)
{
    ...
    {
        sm_log(SM_LOG_DEBUG, "flash_area_write, from: 0x%08x  len: 0x%08x\r\n", addr, len);
        // sm_log(SM_LOG_DEBUG, "flash_area_write ,write: 0x%08x 0x%08x %p\r\n", addr, len, src);
...
int flash_area_erase(const struct flash_area *fa, uint32_t off, uint32_t len)
{
    ...
    sm_log(SM_LOG_DEBUG, "fa->fa_device_id %d, and FLASH_DEVICE_INTERNAL_FLASH is %d and FLASH_DEVICE_EXTERNAL_FLAG is %d\r\n", fa->fa_device_id, FLASH_DEVICE_INTERNAL_FLASH, FLASH_DEVICE_EXTERNAL_FLAG);
    ...
    {
	    /* commented by vincent.he, 20250211, for cancle erase ext-flash total OTA secondary slot(0xC0000)
        ** */
        // rc = ota_smif_erase(addr, len);
        sm_log(SM_LOG_DEBUG, "flash_area_erase: rc 0x%x addr: 0x%08x  len: 0x%08x\r\n", rc, addr, len);
    }
...
flash_area_boot_set_confirmed(void)
{
    ...
    {
        sm_log(SM_LOG_DEBUG, "in flash_area_boot_set_confirmed, ota_smif_initialize ok");
    }
    else
    {
        sm_log(SM_LOG_DEBUG, "in flash_area_boot_set_confirmed, ota_smif_initialize fail");
    }
