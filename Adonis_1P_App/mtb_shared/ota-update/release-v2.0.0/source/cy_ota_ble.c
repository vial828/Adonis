...
cy_rslt_t cy_ota_ble_download_prepare(cy_ota_context_ptr ctx_ptr, uint16_t bt_conn_id, uint16_t bt_config_descriptor)
{
    ...
    /* clear out the stats */
    ota_ctx->total_image_size    = 0;
    ota_ctx->total_bytes_written = 0;
    ota_ctx->last_offset         = 0;
    ota_ctx->last_size           = 0;
    ota_ctx->storage_loc         = NULL;
    ota_ctx->ota_up_type         = CY_OTA_UNKNOW_TYPE;
	ota_ctx->ota_image_earse_total = 0;
...
cy_rslt_t cy_ota_ble_download(cy_ota_context_ptr ctx_ptr, wiced_bt_gatt_event_data_t *p_req, uint16_t bt_conn_id, uint16_t bt_config_descriptor)
{
    ...
    //wanggankun@2024-11-20, add, 须减去头部的8字节标签，不然百分比会计算错误
    ota_ctx->total_image_size = (total_size-7);

    sm_log(SM_LOG_DEBUG, "ota_ctx->total_image_size: %d\r\n", ota_ctx->total_image_size);
...
cy_rslt_t cy_ota_ble_download_write(cy_ota_context_ptr ctx_ptr, wiced_bt_gatt_event_data_t *p_req)
{
	...
	//wanggankun@2024-11-18, add, 判断类型
	if (chunk_info.offset == 0UL)
	{
// 		if( chunk_info.size > 8   &&  strncmp(chunk_info.buffer, "AD2PCASE", 8) == 0)
// 		{
//             //wanggankun@2024-11-20, 拷贝前，先算CRC
// #ifdef  COMPONENT_OTA_BLUETOOTH_SECURE
//             cy_ota_ble_secure_signature_update(ctx_ptr, p_write_req->p_val, p_write_req->offset, p_write_req->val_len);
// #else    
//             ota_ctx->ble.crc32 = crc32_Update(ota_ctx->ble.crc32, (const uint8_t*)(p_write_req->p_val + p_write_req->offset), p_write_req->val_len);
// #endif		
//             have_check_crc = 1;
//             ota_ctx->ota_up_type = CY_OTA_UNKNOW_TYPE; //CY_OTA_UP_BIN;    //Adonis1P should not support upgrade AD2PCASE FW, modifed by zshe
//             chunk_info.size -= 8;
//             full_write_size -= 8;
//             memcpy(&chunk_info.buffer[0], &chunk_info.buffer[8], chunk_info.size);
//             sm_log(SM_LOG_DEBUG, "This is 2P case, size is: %d\r\n", chunk_info.size);
...
// 		    }
// 		}
// 		else if( chunk_info.size > 8   &&  strncmp(chunk_info.buffer, "AD2PUPEN", 8) == 0)
// 		{
//             //wanggankun@2024-11-20, 拷贝前，先算CRC
// #ifdef  COMPONENT_OTA_BLUETOOTH_SECURE
//             cy_ota_ble_secure_signature_update(ctx_ptr, p_write_req->p_val, p_write_req->offset, p_write_req->val_len);
...
// #endif
//             have_check_crc = 1;
// 			ota_ctx->ota_up_type = CY_OTA_UNKNOW_TYPE; //CY_OTA_PEN_UP_BIN;    //Adonis1P should not support upgrade AD2PUPEN FW, modifed by zshe
//             chunk_info.size -= 8;
//             full_write_size -= 8;
//             sm_log(SM_LOG_DEBUG, "This is 2P pen, size is: %d\r\n", chunk_info.size);
//             memcpy(&chunk_info.buffer[0], &chunk_info.buffer[8], chunk_info.size);
// 		}
        if( chunk_info.size > 13/*11*/   &&  strncmp(chunk_info.buffer, "AD1P_UI_IMAGE", 13/*11*/) == 0) //AD1P_UI_IMAGE(12.31 byMIKE) GLO_AD1P_UI(10.11)
		{
			ota_ctx->ota_up_type = CY_OTA_UI_IMAGE;
            sm_log(SM_LOG_DEBUG, "This is 1P UI image, size is: %d Total_size is %d\r\n", chunk_info.size, ota_ctx->total_image_size);
			cy_ota_ui_storage_open(ota_ctx);
		}
        else if( chunk_info.size > 7   &&  strncmp(chunk_info.buffer, "AD1P_UP", 7) == 0) 
        {
            //wanggankun@2024-11-20, 拷贝前，先算CRC
#ifdef  COMPONENT_OTA_BLUETOOTH_SECURE
            cy_ota_ble_secure_signature_update(ctx_ptr, p_write_req->p_val, p_write_req->offset, p_write_req->val_len);
...
#endif		
            have_check_crc = 1;
            ota_ctx->ota_up_type = CY_OTA_UP_BIN;
            chunk_info.size -= 7;
            full_write_size -= 7;
            memcpy(&chunk_info.buffer[0], &chunk_info.buffer[7], chunk_info.size);
			sm_log(SM_LOG_DEBUG, "This is 1P UP bin, size is: %d Total_size is %d\r\n", chunk_info.size, ota_ctx->total_image_size);

            /**
             * Added by vincent.he, for OTA upgrade version control.2025.02.12 
             */
#define  OTA_APP_VERSION_MAJOR_OFFSET (20) //?=2
#define  OTA_APP_VERSION_MINOR_OFFSET (OTA_APP_VERSION_MAJOR_OFFSET+1) //?=0
#define  OTA_APP_VERSION_BUILD_OFFSET (OTA_APP_VERSION_MINOR_OFFSET+1) //?=5
			sm_log(SM_LOG_DEBUG, "Current version: %d.%d.%d ,", APP_VERSION_MAJOR, APP_VERSION_MINOR, APP_VERSION_BUILD);
			sm_log(SM_LOG_DEBUG, "Upgrade version: %d.%d.%d \r\n", chunk_info.buffer[OTA_APP_VERSION_MAJOR_OFFSET],
                                                                   chunk_info.buffer[OTA_APP_VERSION_MINOR_OFFSET],
                                                                   chunk_info.buffer[OTA_APP_VERSION_BUILD_OFFSET]);
            /**
             * refuse version5.1.0 to OTA upgrade
            */
            if ((chunk_info.buffer[OTA_APP_VERSION_MAJOR_OFFSET] == 5)&&
                (chunk_info.buffer[OTA_APP_VERSION_MINOR_OFFSET] == 1)&&
                (chunk_info.buffer[OTA_APP_VERSION_BUILD_OFFSET] == 0))
            {
                return CY_RSLT_OTA_ERROR_BLE_GATT;
            }
            /** version control */
            if (chunk_info.buffer[OTA_APP_VERSION_MAJOR_OFFSET] > APP_VERSION_MAJOR)
            {
                //;
            }
            else if (chunk_info.buffer[OTA_APP_VERSION_MAJOR_OFFSET] == APP_VERSION_MAJOR)
            {
                if (chunk_info.buffer[OTA_APP_VERSION_MINOR_OFFSET] > APP_VERSION_MINOR)/** miss "else" ,priority should MAJOR>MINOR>BUILD  */
                {
                    //;
                }
                else if (chunk_info.buffer[OTA_APP_VERSION_MINOR_OFFSET] == APP_VERSION_MINOR)/** miss "else" ,priority should MAJOR>MINOR>BUILD  */
                {
                    if (chunk_info.buffer[OTA_APP_VERSION_BUILD_OFFSET] < APP_VERSION_BUILD)
                    {
                        return CY_RSLT_OTA_ERROR_BLE_GATT;
                    }
                }
        ...
        {
            ota_ctx->ota_up_type = CY_OTA_UNKNOW_TYPE; //not support other FW upgrade, bug1985800
        }
	}

    //wanggankun@2024-11-18, add, 判断类型出错，返回
    if (ota_ctx->ota_up_type == CY_OTA_UNKNOW_TYPE)
    {
        sm_log(SM_LOG_DEBUG, "Total_size is %d, error type!!!!\r\n", ota_ctx->total_image_size);
        return CY_RSLT_OTA_ERROR_BLE_GATT;
    }

	if (chunk_info.size > 0)
	{
		if (ota_ctx->ota_up_type == CY_OTA_PEN_UP_BIN)
		{
			result = CY_RSLT_SUCCESS;
		}
		else if(ota_ctx->ota_up_type == CY_OTA_UP_BIN)
		{
            // sm_log(SM_LOG_DEBUG, "Offset: 0x%x  Size is: %d  \n", chunk_info.offset, chunk_info.size);
			result = cy_ota_write_incoming_data_block(ctx_ptr, &chunk_info);
		}
		else if (ota_ctx->ota_up_type == CY_OTA_UI_IMAGE)
		{
			result = cy_ota_storage_ui_write(ota_ctx, &chunk_info);
		}

		if (result != CY_RSLT_SUCCESS)
		{
			cy_rtos_setbits_event(&ota_ctx->ota_event, (uint32_t)CY_OTA_EVENT_DATA_FAIL, 0);
			cy_log_msg(CYLF_OTA, CY_LOG_ERR, "     cy_ota_ble_download_write() FAILED : 0x%lx \n", result);
			return CY_RSLT_OTA_ERROR_BLE_GATT;
		}
	}
    ...
    //sm_log(SM_LOG_DEBUG, "Downloaded 0x%lx of 0x%lx (%d%%), ble.crc32: 0x%x\r\n", ota_ctx->total_bytes_written, ota_ctx->total_image_size, ota_ctx->ble.percent, ota_ctx->ble.crc32);
...
cy_rslt_t cy_ota_ble_download_verify(cy_ota_context_ptr ctx_ptr, wiced_bt_gatt_event_data_t *p_req, uint16_t bt_conn_id)
{
    ...
    if (p_write_req->val_len == 5)
    {
        ...
        if (ota_ctx->ble.crc32 != ota_ctx->ble.received_crc32)
        {
            ...
            sm_log(SM_LOG_ERR,"     check CRC FAILED 0x%lx != 0x%lx\n", ota_ctx->ble.crc32, ota_ctx->ble.received_crc32);
    ...
    {
        sm_log(SM_LOG_ERR,"      check CRC - len from Host !=5 %d\n", p_write_req->val_len);
    ...
    if (result == CY_RSLT_SUCCESS)
    {
    	...
    	if (ota_ctx->ota_up_type == CY_OTA_UP_BIN)
        {
    	...
    	else if (ota_ctx->ota_up_type == CY_OTA_PEN_UP_BIN)
    	{
    		sm_log(SM_LOG_DEBUG, "this is the pen image, write pen\r\n");
    		result = CY_RSLT_SUCCESS;
    	}
		else if (ota_ctx->ota_up_type == CY_OTA_UI_IMAGE)
    	{
    		result = cy_ota_storage_ui_verify(ota_ctx);
            if (result != CY_RSLT_SUCCESS)
			{				
				result = CY_RSLT_OTA_ERROR_BLE_VERIFY;
			}
    ...
    {
        ota_ctx->ble.bt_notify_buff = CY_OTA_UPGRADE_STATUS_VERIFY_FAILED;//before: CY_OTA_UPGRADE_STATUS_BAD， modify as CY_OTA_UPGRADE_STATUS_VERIFY_FAILED
        cy_log_msg(CYLF_OTA, CY_LOG_ERR, "     call wiced_bt_gatt_send_indication(%d) !\n", ota_ctx->ble.bt_notify_buff);
        sm_log(SM_LOG_DEBUG, "call wiced_bt_gatt_send_indication(%d) !\r\n", ota_ctx->ble.bt_notify_buff);
    }
