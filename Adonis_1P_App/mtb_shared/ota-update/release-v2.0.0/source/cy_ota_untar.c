...
cy_rslt_t cy_ota_write_incoming_data_block(cy_ota_context_ptr ctx_ptr, cy_ota_storage_write_info_t * const chunk_info)
{
    ...
    if (chunk_info->offset == 0UL)
    {
        ...
        {//wanggankun@2025-2-25, note, 这里不是tar header。如果后面是，这里要小心
    ...
    else
    {
        ...
        //wanggankun@2025-2-25, add, 升级时判断是否要擦除FLASH
        ...
        {
           ext_flash_earse( fap->fa_off + (data_will_be_written/4096)*4096, 4096);
        }
        //end add
